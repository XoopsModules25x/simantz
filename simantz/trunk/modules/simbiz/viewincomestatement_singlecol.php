	...
	//$periodrange=$financialyear->getPeriodRange($financialyearfrom_id);
      ...
      //  $dateto='2008-10-31';
       ...
       $allsubsql.="case when (SELECT sum(t.amt) FROM $tabletransaction t
                     ...
                     b.batchdate BETWEEN '$datefrom' and '$dateto') is null
                    then 0 else
                        (SELECT sum(t.amt) FROM $tabletransaction t
                        inner join $tablebatch b on b.batch_id=t.batch_id
                        where b.iscomplete=1 and t.accounts_id=a.accounts_id and t.branch_id = $organization_id and 
                        b.batchdate BETWEEN '$datefrom' and '$dateto') end";
...
//	else
	...
	while($row=$xoopsDB->fetchArray($query)){
		...
		$transactionamt=$row['transactionamt'];
                if($showaccountcode=="on")
                    $displaytext=$accountcode_full . "-".$displaytext;
	  ...
	  if($currentcategory !=$classtype){
		...
		switch($currentcategory){
		  case "1S":
			$summaryamt=changeNegativeNumberFormat($totalsales*-1,2);
			$pdf->Cell($w[1],$defaultfontheight,$summaryamt,0,0,"R");
		  break;	
		  case "2C":
			$summaryamt=changeNegativeNumberFormat($totalcost,2);
			$pdf->Cell($w[1],$defaultfontheight,$summaryamt,0,0,"R");

			$pdf->Ln();
			$pdf->SetFont($defaultfont,'B',$defaultfontsize+1);
			$grossprofit=$totalsales*-1 -$totalcost;

			if($grossprofit < 0){
			 $pdf->Cell($w[0],$defaultfontheight,"Gross Loss",0,0,"L");
			 $pdf->Cell($w[1],$defaultfontheight,"".changeNegativeNumberFormat($grossprofit,2)."",0,0,"R");
		  ...
		  break;	
		  case "3O":
			$summaryamt=changeNegativeNumberFormat($totalotherincome*-1,2);
			$pdf->Cell($w[1],$defaultfontheight,$summaryamt,0,0,"R");
...
//	if(substr($displaytext,0,1) == "[" || $checkzero <> 0){
	  ...
	  $pdf->Ln();
		...
		//display summary for last row
			$ypos=$pdf->GetY();		
			$pdf->Line($w[0],$ypos,$marginx+$w[0]+$w[1],$ypos);
			$summaryamt=changeNegativeNumberFormat($totalexpenses,2);
			$pdf->Cell($w[0],$defaultfontheight,"",0,0,"R");
			$pdf->Cell($w[1],$defaultfontheight,$summaryamt,0,0,"R");
			$pdf->Ln();
			$pdf->Ln();
			$pdf->SetFont($defaultfont,'B',$defaultfontsize+1);
			$netprofit=$grossprofit-$totalexpenses + -1*$totalotherincome;
                        
//($totalotherincome)*(-1) -
			if($netprofit < 0){
			 $pdf->Cell($w[0],$defaultfontheight,"Net Loss",0,0,"L");
			 $pdf->Cell($w[1],$defaultfontheight,"".changeNegativeNumberFormat($netprofit,2)."","TB",0,"R");
			}
			else{
