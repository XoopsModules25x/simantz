...
//	$pdf=new PDF('P','mm',$papertype[$statementpapersource]);  //0=A4,1=Letter
  ...
  $hierarchy=explode(']',$row['hierarchy']);
                ...
                $lastbalance=$acc->getAccountDateRangeValue($datefrom,$dateto,$accounts_id,0);
		...
		switch($classtype){
		  ...
		  break;	
		  case "6L":
			$totalliability=$totalliability+abs($lastbalance);
                        if($placeholder==0 )
                            $displayamt=changeNegativeNumberFormat($lastbalance,2);
                        elseif($placeholder==1  && $treelevel<$reportlevel)
                            $displayamt="";
		  ...
		  case "7E":
			...
			$totalequity=$totalequity+abs($lastbalance);
                        if($placeholder==0 )
                            $displayamt=changeNegativeNumberFormat($lastbalance,2);
                        elseif($placeholder==1  && $treelevel<$reportlevel)
                            $displayamt="";
	  ...
	  if($currentcategory !=$classtype){
		...
		switch($currentcategory){
		  ...
		  case "6L":
			...
			$summaryamt=changeNegativeNumberFormat($totalliability,2);
			$pdf->Cell($w[1],$defaultfontheight,$summaryamt,0,0,"R");
...
*/
		  break;	
		  case "7C":
			$summaryamt=changeNegativeNumberFormat($totalequity,2);
			$pdf->Cell($w[1],$defaultfontheight,$summaryamt,0,0,"R");
	  ...
	  $data=array($prefix.$displaytext,"$bracket $displayamt");
	  $pdf->SetX($marginx);
	...
	//if(substr($displaytext,0,1) == "[" || $checkzero <> 0){
	  ...
	  $pdf->Ln();
		...
		//display summary for last row
			...
			$pdf->SetFont($defaultfont,'B',$defaultfontsize+1);
			$totaleq_lib =$totalequity+$totalliability;
			$pdf->Cell($w[0],$defaultfontheight,"Total Liability + Equity:",0,0,"L");
			$pdf->Cell($w[1],$defaultfontheight,changeNegativeNumberFormat($totaleq_lib,2),"TB",0,"R");
			$pdf->Ln();
	...
	$pdf->Output("viewbalancesheet_singleperiod.pdf","I");
	exit (1);
