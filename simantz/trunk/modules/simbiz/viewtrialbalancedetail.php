...
include_once "system.php";
	$header=array("No","Date","Batch","A/C No","A/C Name","A/C Group","Debit","Credit");
   	$w=array(10,17,17,17,60,29,20,20);
...
//	$pdf->accounts_codeto=getAccountsID($_POST['accounts_codeto']);
	...
	$wherestr="(b.batchdate between '$pdf->datefrom' AND '$pdf->dateto') AND 
			...
			and b.batch_id>0 and b.batchno>=0 and a.accounts_id>0 and b.iscomplete=1 and b.organization_id=$defaultorganization_id";
	...
	$sql="SELECT b.batchdate,a.accounts_id, a.accountcode_full,a.accounts_name,g.accountgroup_name, 
		t.amt as amt,a.account_type,bp.bpartner_name,b.batchno
		FROM $tablebatch b 
		INNER JOIN $tabletransaction t on b.batch_id=t.batch_id
		INNER JOIN $tableaccounts a on a.accounts_id=t.accounts_id
		INNER JOIN $tablebpartner bp on bp.bpartner_id=t.bpartner_id
		INNER JOIN $tableaccountgroup g on a.accountgroup_id=g.accountgroup_id 
		WHERE $wherestr
		order by b.batchdate,b.created,a.accountcode_full";	
	...
	while($row=$xoopsDB->fetchArray($query)){
		...
		$pdf->Cell($w[1],$pdf->tabletextheight,$batchdate,0,0,'L');
		$pdf->Cell($w[2],$pdf->tabletextheight,$batchno,0,0,'L');		
		$pdf->Cell($w[3],$pdf->tabletextheight,$accountcode_full,0,0,'L');
		$displayname=$accounts_name;
		if($account_type==2 || $account_type==3)
		$displayname=$bpartner_name;
		$pdf->Cell($w[4],$pdf->tabletextheight,$displayname,0,0,'L');
		$pdf->Cell($w[5],$pdf->tabletextheight,$accountgroup_name,0,0,'L');
		$pdf->Cell($w[6],$pdf->tabletextheight,$debitamt,0,0,'R');
		$pdf->Cell($w[7],$pdf->tabletextheight,$creditamt,0,0,'R');
...
//		$log->showLog(5,"$accountcode_full");
		...
		$pdf->Cell($w[5],$pdf->tabletextheight,"","TB",0,'L');
		$pdf->Cell($w[6],$pdf->tabletextheight,number_format($totaldebit,2),"TB",0,'R');
		$pdf->Cell($w[7],$pdf->tabletextheight,number_format($totalcredit,2),"TB",0,'R');
	//$pdf->AddPage();
	//$pdf->BasicTable($data);
