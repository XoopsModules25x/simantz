  ...
  public function getInputForm( $type,  $accounts_id,$token  ) {
	...
	else
	{
		...
		"</form>";
                ...
                $hyperlink= "<a href='viewsingleledger.php?submit=submit&accounts_id=$this->accounts_id&periodfrom_id=0&periodto_id=0&showbatchno=on'  target='_blank'>[View Ledger Report]</a>
                <a href='batch.php?defaultaccounts_id=$this->accounts_id&action=new' target='_blank'>[Add Transaction]</a>
                <a href='transactionsummary.php?accounts_id=$this->accounts_id' target='_blank'>[View Transaction Summary]</a>
                <a href='viewtransaction.php?accounts_id=$this->accounts_id' target='_blank'>[View Transaction Report(Include Draft)]</a>";
                else
            $hyperlink="<a href='accounts.php?action=addchild&parentaccounts_id=$this->accounts_id&accountgroup_id=$this->accountgroup_id&parentaccounts_code=$this->accounts_code'>".
            "[Add Child Account]</a>";
		...
		switch($this->account_type){
			...
			break;
                        case "8":
				$account_typectrl="<input name='accountype_text' value='Retain Earning (Reverse Entry)' readonly>";
                         break;
...
<input name='previousopeningbalance' value=$this->openingbalance $stylelevel1>
<input name='openingbalance' value=$this->openingbalance $stylelevel1  type='hidden'></div>
      <tr>
...
('$timestamp', $userid, '$timestamp', $userid, '1', 'Retain Earning(Reverse Entry)',         $memo, '0.00', $memo_id, 0, 0, '0.00', $organization_id, 0, 10, '', 2, 8,'81');
...
/*
 ...
 public function getAccountDateRangeValue($datefrom,$dateto,$accounts_id,$bpartner_id){
...
public function showAccountsTree($wherestring,$orderstring){
    $children = 0;
    $no_children = array();
  $this->lastaccounts_id=0;
  $this->chartaccountparentaccountlist="";
  echo "<A onclick='document.getElementById(\"idshowhideall\").onclick()'>Show/Hide All</a>";
    $this->display_children(0,0,$wherestring,$orderstring);
    echo <<< EOF
<style>
	li{
		list-style:none;
...
function show(subCategory, path){
	...
	if(thesub.style.display == 'block'){
		...
		theimage.src = path+'images/openplaceholder.gif';
		...
		*/
		...
		theimage.src = path+'images/closeplaceholder.gif';
...
function showAllItem(list,path){
var id=list.split(",");
...
EOF;
if($this->isAdmin)
   $repairaccctrl='<input type="button" value="Repair Account" onclick="repairAccounts()">';
echo "<br/><br/>".$repairaccctrl.
"<br/> * Warning! This is a heavy activity, <br/>use only when found incosistency.";
$this->chartaccountparentaccountlist=substr_replace($this->chartaccountparentaccountlist,"",-1);
echo "<A id='idshowhideall' onclick='showAllItem(\"$this->chartaccountparentaccountlist\",\"\")'>Show/Hide All</a>";
echo "<input type='button' value='Recheck Hierarchy' onclick='updateHierarchy()'>";
}

private function display_children($parent, $level,$wherestring,$orderstring) {
	global $children;
	global $no_children;

       
   // retrieve all children of $parent
 $sql = sprintf("SELECT accounts_id,accountcode_full, accounts_name,placeholder,accounts_code,
                        accountgroup_id
                        FROM $this->tableaccounts WHERE parentaccounts_id = %d $wherestring
                        $orderstring",
                        $parent);
   ...
   while ($row =$this->xoopsDB->fetchArray($result)) {
   		...
   		$list_id = 'list_'.$children;
                $accounts_id=$row['accounts_id'];
                if($this->lastaccounts_id<$accounts_id)
                    $this->lastaccounts_id=$accounts_id;
   ...
   $hyperlink="";
    ...
    $hyperlink="&nbsp;&nbsp;&nbsp;".
            "<a href='accounts.php?action=addchild&parentaccounts_id=$accounts_id&accountgroup_id=$accountgroup_id&parentaccounts_code=$accounts_code'>".
...
//
	...
	for($j=0;$j<$level;$j++)
        ...
        if($row['placeholder']==1){
            echo '<a  onClick="show('.$accounts_id.', \''.$HTTP_PATH.'\') ">';
            echo '<img src="'.$HTTP_PATH.'images/openplaceholder.gif" title="expand" border="0" id="img_'.$row['accounts_id'].'">';
           

            }
        else{
               echo '<a href="#" >';
            echo '<img src="'.$HTTP_PATH.'images/empty.gif" title="expand" border="0" id="img_'.$row['accounts_id'].'">';
           
        }
        

         echo '</a>&nbsp;&nbsp;&nbsp;';
	   echo "<A href='accounts.php?action=edit&accounts_id=$accounts_id'>".$row['accountcode_full']." ".$row['accounts_name']."</a>".$hyperlink;  //." <span style='color:red;'>".$list_id."</span>";
	   echo '</li>';

       // call this function again to display this child's children
       $this->display_children($row['accounts_id'], $level+1,$wherestring,$orderstring);
   }
...
public function getRetainEarningAccount(){
    global $defaultorganization_id;
   
   echo $sql="SELECT accounts_id,account_type FROM $this->tableaccounts
        where organization_id=$defaultorganization_id
        AND account_type=6 or account_type=8 order by account_type ASC";
     $query=$this->xoopsDB->query($sql);
     $account=array();
     while($row=$this->xoopsDB->fetchArray($query)){
     $account[]=$row['accounts_id'];
     }
     return $account;
             //Retain Earning (type=6)
        //Retain Earning(Reverse Entry) (type=8)
