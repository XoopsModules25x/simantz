...
//constructor
   public function AccountGroup(){
	global $xoopsDB,$log,$tableaccountgroup,$tablebpartner,$tablebpartnergroup,$tableorganization,$tableaccounts;
 ...
 action="accountgroup.php" name="frmAccountGroup"><input name="reset" value="Reset" type="reset"></td></tbody></table>
  ...
  <table style="text-align: left; width: 100%;" border="1" cellpadding="0" cellspacing="1">
    <tbody>
      ...
      <tr>
        <td class="head">Initial No</td>
        <td class="odd"><input maxlength="1" size="1" name="initial" value="$this->initial"></td>
...
EOF;
  ...
  public function updateAccountGroup( ) {
	...
	organization_id=$this->organization_id,initial=$this->initial WHERE accountgroup_id='$this->accountgroup_id'";
  ...
  public function insertAccountGroup( ) {
   ...
   $timestamp= date("y/m/d H:i:s", time()) ;
	...
	updated,updatedby,defaultlevel,organization_id,description,accountclass_id,initial) values(
	'$this->accountgroup_name','$this->isactive','$timestamp',$this->createdby,'$timestamp',
	$this->updatedby,$this->defaultlevel,$this->organization_id,'$this->description',$this->accountclass_id,$this->initial)";
  ...
  public function fetchAccountGroup( $accountgroup_id) {
	...
	$sql="SELECT accountgroup_id,accountgroup_name,isactive,defaultlevel,organization_id,description,accountclass_id,initial
  ...
  $this->log->showLog(4,"Running ProductAccountGroup->getSQLStr_AllAccountGroup: $sql");
    ...
    $sql="SELECT accountgroup_name,accountgroup_id,isactive,organization_id,defaultlevel,initial,accountclass_id FROM $this->tableaccountgroup " .
 ...
 public function showAccountGroupTable($wherestring,$orderbystring){
	...
	<table border='1' cellspacing='3'>
  		<tbody>
				...
				<th style="text-align:center;">Initial No</th>
...
EOF;
	...
	while ($row=$this->xoopsDB->fetchArray($query)){
		...
		<tr>
			...
			<td class="$rowtype" style="text-align:center;">$initial</td>
...
/**
 ...
 public function allowDelete($accountgroup_id){
	$sql="SELECT count(account_id) as rowcount from $this->tableaccounts where accountgroup_id=$accountgroup_id";
...
public function getSelectAccountGroup($accountgroup_id){
    global $defaultorganization_id;
      $sql="SELECT * FROM $this->tableaccountgroup where organization_id=$defaultorganization_id
                    or ( accountgroup_id=$accountgroup_id and accountgroup_id>0) order by defaultlevel,accountgroup_name";
    $query=$this->xoopsDB->query($sql);
    $result="";
    while($row=$this->xoopsDB->fetchArray($query)){
    ...
    $result.="<option value='$accountgroup_id' $selected>$accountgroup_name</option>";
