...
function pageHeader(){
 ...
 <h1 style='text-align: center'>Chart Generator From Balance Sheet</h1><br>
...
EOF;
	...
	foreach($arrayperiod as $id){
	     ...
	     $monthid[$m]=$id;
             ...
             $allsubsql.="case when (SELECT sum(t.amt) FROM $tablebatch b
                    ...
                    where t.accounts_id=a.accounts_id and  b.batchdate <='".$lastdate[$m]. "' and b.iscomplete=1) is null
		then 0 else (SELECT sum(t.amt) FROM $tablebatch b
                    inner join $tabletransaction t on t.batch_id=b.batch_id
                    where t.accounts_id=a.accounts_id and b.batchdate <='".$lastdate[$m]. "' and b.iscomplete=1) end
		 as transactionamt".$m.",";
	...
	//if display folder indicator = 1, account with placeholder=1 will display as [account]
	  ...
	  elseif($placeholder==1  && $treelevel<$reportlevel){
		...
		$transactionamt[$k]=$acc->getAccountDateRangeValue($datefrom,$dateto,$accounts_id,0);
