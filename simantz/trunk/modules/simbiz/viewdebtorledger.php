...
function Header()
{
	...
	$this->Cell($this->headertitlewitdth,$this->headertitleheight,"Debtor Ledger",0,0,'L');
...
//	$this->SetFont('Times','',10);   
	...
	$this->Cell($this->ledgertitlecolwidth,$this->ledgertitleheight,
		"Debtor No: $this->headerbpartner_no-$this->headerbpartner_name",0,0,'L');
	$this->Ln();
...
// 	$pdf->dateto=$dateto;
	...
	$sqlgetaccountlist="SELECT a.accounts_id,a.accountcode_full,bp.bpartner_no,bp.bpartner_name,bp.bpartner_id
		...
		INNER JOIN $tablebpartner bp on bp.debtoraccounts_id=a.accounts_id
		WHERE  placeholder=0 and bp.organization_id=$defaultorganization_id and 
		a.account_type=2 and bp.isdebtor=1 and
	...
	$accounts_id=$rowbpartner['accounts_id'];
...
//		order by a.accountcode_full,b.batchdate,b.batchno,t.seqno,t.reference_id desc
	//get required data 		and a1.accounts_id=$accounts_id 		and a1.accounts_id=$accounts_id
	$sql="SELECT a.batchdate,a.batchno,a.accountcode_full,a.accounts_id,a.accounts_name,a.document_no,a.amt,
		...
		WHERE b.batchdate between '$datefrom' and '$dateto' and t1.bpartner_id=$bpartner_id and b.iscomplete=1  and a1.account_type=2
		...
		t2.amt as refamt,a1.accounts_name as refaccounts_name,a1.accounts_code as refaccounts_code,
		b.batch_id,t2.document_no2,t1.document_no2 as refdocument_no2,t1.linedesc 
		FROM $tablebatch b  
		INNER JOIN $tabletransaction t1 on b.batch_id=t1.batch_id 
		INNER JOIN $tableaccounts a1 on a1.accounts_id=t1.accounts_id 
		INNER JOIN $tabletransaction t2 on t1.reference_id=t2.trans_id 
		INNER JOIN $tableaccounts a2 on a2.accounts_id=t2.accounts_id 
		WHERE b.batchdate between '$datefrom' and '$dateto' and t2.bpartner_id=$bpartner_id and b.iscomplete=1 and a2.account_type=2
...
;
	...
	$balanceamt=$acc->bPartnerBalanceBFAmount($periodfrom_id,$bpartner_id,2);
	...
	$pdf->Cell($pdf->ledgertitlecolwidth,$pdf->ledgertitleheight,
		"Debtor No: $pdf->headerbpartner_no-$pdf->headerbpartner_name",0,0,'L');
	$pdf->Ln();
