...
class PDF extends FPDF
{
  ...
  public $netpayamt;
  public $columnwidth=95;
  public $headerheight=6;
  public $itemheight=5;
  public $startypos=40;
  public $startxpos=10;
  public $containheight=70;
...
function Header()
{
   ...
   // $this->Image('./images/simonlogobk.jpg', 11 , 6 , 90 , '' , 'JPG' , 'payment.php');
	$this->Image('../../images/logo.jpg',10,5,20,'','JPG','payslip.php');
	$this->SetFont('Times','B',15);
	$this->setXY(30,11);
	$this->Cell(93,1,$this->organization_name,10,10,'L');
        $this->Cell(93,1,$this->companyno,0,0,'L');


	$this->SetFont('Times','',7);
	$this->setXY(30,16);
				
	$this->Cell(93,4,"$this->street1 $this->street2 $this->street3 $this->postcode $this->city  $this->state_i.  Tel: $this->tel_1, ".
	"Fax: $this->fax",0,0,'L');
	$this->setXY(105,14);
	...
	$this->MultiCell(190,4,"NAME                : $this->employee_name",1,'L');
	$this->MultiCell(190,4,"DEPARTMENT : $this->department",1,'L');
	$this->MultiCell(190,4,"DESIGNATION : $this->position",1,'L');
	...
	$this->MultiCell(50,4,"NRIC                   : " . $this->employee_newicno  ,1,'L');
	$this->setX($x2startpos);
	$this->MultiCell(50,4,"EMPLOYEE NO : " . $this->employee_no  ,1,'L');
	$this->setX($x2startpos);
	$this->MultiCell(50,4,"SOCSO NO         : " . $this->employee_socsono  ,1,'L');


	$this->setXY($x3startpos,$ystartpos);
	$this->MultiCell(60,4,"PAYSLIP FOR  :  $this->datefrom / $this->dateto"  ,1,'L');
	$this->setX($x3startpos);
	$this->MultiCell(60,4,"EPF NO                   : " . $this->employee_epfno  ,1,'L');
	$this->setX($x3startpos);
	$this->MultiCell(60,4,"INCOME TAX NO : " . $this->employee_taxno  ,1,'L');
//	$this->setX($x3startpos);
//	$this->MultiCell(70,4,"Date From/To  :  $this->datefrom / $this->dateto"  ,0,'L');
	...
	//$this->Rect($columnwidth-22, $startypos+$headerheight,22,$this->headerheight );
	...
	//$this->Rect($startxpos+$columnwidth * 2, $headerheight+$startypos,$columnwidth,$containheight );
        ...
        $this->SetFillColor(236,220,220);
        
	$this->setXY($startxpos,$startypos);
	$this->SetFont('Times','B',10);
	$this->Cell($columnwidth,$headerheight,"EARNINGS",1,0,'C',1);
	$this->Cell($columnwidth,$headerheight,"DEDUCTIONS",1,0,'C',1);
	//$this->Cell($columnwidth,$headerheight,"Others (RM)",0,0,'C',0);
	...
	$this->SetFont('Times','B',10);
	$this->Cell($columnwidth-22,$headerheight,"GROSS PAY ( A )",1,0,'L',1);
        $this->Cell(22,$headerheight,"$this->totalincomeamt",0,0,'R',0);
	$this->Cell($columnwidth-22,$headerheight,"DEDUCTIONS ( B )",1,0,'L',1);
        $this->Cell(22,$headerheight,"$this->totaldeductamt",0,1,'R',0);
        
        $this->Cell($columnwidth,$headerheight,"EMPLOYER CONTRIBUTIONS",1,0,'C',1);
	$this->Cell($columnwidth-22,$headerheight,"NET PAY ( A ) - ( B )",1,0,'L',1);
        $this->Cell(22,$headerheight,"$this->netpayamt",1,1,'R',0);
        $this->Cell($columnwidth/2,$headerheight,"EPF : $this->employer_epfamt",1,0,'L',0);
        $this->Cell($columnwidth/2,$headerheight,"SOCSO : $this->employer_socsoamt",1,0,'L',0);
        $this->Cell($columnwidth,$headerheight,"ACKNOWLEDGEMENT : ",1,0,'L',0);
...
$this->Rect($startxpos+$columnwidth-$w[2], $startypos,$w[2],$this->containheight+$this->headerheight );

$this->SetFillColor(236,220,220);

$this->SetFont('Times','',8);
$this->setX($startxpos);
$this->Cell($w[0]+$w[1],$itemheight,"PARTICULARS",1,0,"L",1);
$this->Cell($w[2],$itemheight,"AMOUNT ( RM )",1,1,"C",1);
...
$this->SetFont('Times','',9);
  foreach($row as $col){
	...
	if($i==0)
	$this->setX($curxpos);
	$this->Cell($w[$i],$itemheight,$col,$bordertbl,0,$alignment);
...
$pdf=new PDF('P','mm', 'A4');
	...
	$sql="SELECT p.payslip_id,e.employee_id, e.employee_no, e.employee_name, e.employee_newicno, e.employee_hpno, e.permanent_telno as tel,
		...
		p.basicsalary, commissionamt, hourlycommisionamt,employee_epfamt,e.employee_epfno,e.employee_taxno,e.employee_socsono,
	...
	if($rowheader=$xoopsDB->fetchArray($query)){
		...
		$pdf->employee_taxno=$rowheader['employee_taxno'];
		...
		$pdf->employer_epfamt =$rowheader['employer_epfamt'];
		$pdf->employer_socsoamt =$rowheader['employer_socsoamt'];
