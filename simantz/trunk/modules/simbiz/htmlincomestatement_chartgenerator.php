...
function pageHeader(){
 ...
 <h1 style='text-align: center'>Chart Generator For Income Statement</h1><br>
	...
	foreach($arrayperiod as $id){
	     ...
	     $monthid[$m]=$id;
             ...
             $allsubsql.="case when (SELECT sum(t.amt) FROM $tablebatch b
                    ...
                    where t.accounts_id=a.accounts_id and b.period_id=$id and b.iscomplete=1) is null
		then 0 else (SELECT sum(t.amt) FROM $tablebatch b
                    inner join $tabletransaction t on t.batch_id=b.batch_id
                    where t.accounts_id=a.accounts_id and b.period_id=$id and b.iscomplete=1) end
		 as transactionamt".$m.",";
