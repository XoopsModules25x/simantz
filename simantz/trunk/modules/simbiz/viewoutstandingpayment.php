...
<script type="text/javascript">
        ...
        function validate(){
            ...
            else if(action=='printpreview' || action =='studentexam')
                return true;
...
$isexamline=$_POST['isexam'];

$show_oustandingonly=$_POST['show_oustandingonly'];
...
EOF;
  ...
  default :
    ...
    $checkedOutN = "";
    $checkedOutY = "";
    if($show_oustandingonly == "N")
    $checkedOutN = "selected";
    else
    $checkedOutY = "selected";
...
echo <<< EOF
    <form name="frmSearch" action="viewoutstandingpayment.php" method="POST" atarget="_blank">
    ...
    <tr>
    <td class="head">Show Outstanding</td>
    <td class="even" colspan="3">
    <select name="show_oustandingonly">
    <option value="Y" $checkedOutY>Outstanding Only</option>
    <option value="N" $checkedOutN>Show All</option>
    </select>
    </td>
    </tr>
...
EOF;
    ...
    $wherestr = " where si.organization_id = $defaultorganization_id and si.studentinvoice_id > 0 ";

    if($semester_id >0)
    $wherestr .= " and si.semester_id = $semester_id ";
    ...
    $sql = "select si.*,
    (select sum(total_amt) from $tablestudentinvoice
    where student_id = si.student_id and semester_id < st.semester_id
    and iscomplete = 1 ) as total_previousamt,
    (select sum(studentpayment_lineamt) 
    from $tablestudentpayment sp2, $tablestudentpaymentline pl2, $tablestudentinvoice si2, $tablestudentinvoiceline sl2
    where sp2.studentpayment_id = pl2.studentpayment_id
    and pl2.studentinvoiceline_id = sl2.studentinvoiceline_id
    and si2.studentinvoice_id = sl2.studentinvoice_id
    and sp2.student_id = st.student_id
    and si2.semester_id < st.semester_id
    and sp2.iscomplete = 1) as total_previouspaid,

    sum(si.total_amt) as sum_total,
    sum(sp.total_amt) as total_paid,
    st.student_no,st.student_name,cr.course_name,cr.course_no,st.isfinanceexam
    ...
    left join $tablestudentpayment sp on sp.student_id = st.student_id and sp.studentpayment_category = 'I'
    $wherestr
...
echo <<< EOF
        <form name="frmList" action="viewoutstandingpayment.php" method="POST" target="_blank" onsubmit="return validate()">
        <table>
        ...
        <th align="center">Reminder Letter</th>
...
EOF;
        ...
        while($row=$xoopsDB->fetchArray($rs)){
            ...
            $sum_total = $row['sum_total'];
            $total_paid = $row['total_paid'];
            $total_previousamt = $row['total_previousamt'];
            $total_previouspaid = $row['total_previouspaid'];
            $isfinanceexam = $row['isfinanceexam'];


            $total_previousoutstanding = $total_previousamt - $total_previouspaid;
            
            $total_outstanding = $sum_total - $total_paid;


            $isloop = true;
            if($show_oustandingonly == "Y"){
...
echo <<< EOF
        ...
        <input type="hidden" name="total_outstandingline[$i]" value="$total_outstanding">
        <input type="hidden" name="total_previousoutstandingline[$i]" value="$total_previousoutstanding">
        ...
        <a href="studentreminder.php?action=search&issearch=Y&student_id=$student_id" target="blank"><input type="button" name="btnView" value="View Reminder" title="View Student Reminder"></a>
        </td>
        <td class="$rowtype" align="center">
        <a href="studentchase.php?action=search&issearch=Y&student_id=$student_id" target="blank"><input type="button" name="btnViewHist" value="View History" title="View Student Reminder"></a>
        </td>
        <td class="$rowtype" align="center"><input type="checkbox" name="isexam[$i]"$checkedexam></td>
...
echo <<< EOF
        ...
        <input value="Print Preview" type="submit" onclick="action.value='printpreview'">
        <input value="Update Student Exam" type="submit" onclick="action.value='studentexam'">
        </td>
        <td colspan="8" align="right">
        <input value="SMS / Email" type="hidden" aonclick="printForm()" >
        <i>(SMS / Email selected line)</i>
