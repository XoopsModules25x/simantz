   ...
   $sql="SELECT w.window_id,w.window_name,w.filename,w.parentwindows_id,w.seqno,w.windowsetting,
	...
	echo <<< EOF
		...
		<TR>	<TD class="$rowtype">$this->window_no</TD>
			...
			<TD class="$rowtype">$parentname</TD>
			...
			<TD class="$rowtype"><INPUT name="linevaliduntil[$i]" value="$validuntil"></TD>
			<TD class="$rowtype"><INPUT name="linepermissionsetting[$i]"  value="$permissionsetting" title="option:description"></TD>
		</TR>
...
function showMenu($parentwindows_id,$level,$uid,$module_id){
 ...
 $query=$this->xoopsDB->query($sql);
    while($row=$this->xoopsDB->fetchArray($query)){
        ...
        $output .= "<li><a $linkname class='menulink'>".$row['window_name']."</a><ul>";
