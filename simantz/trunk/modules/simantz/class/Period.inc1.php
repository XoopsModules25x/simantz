<?php
include "system.php";
include_once 'class/Jobposition.php';
...
case "search": //return xml table to grid
    $wherestring=" WHERE jobposition_id>0";
    $o->showJobposition($wherestring);
...
if($isadmin==1){
    ...
    $changewidth="width='350'";
  //  $showdeleted="<label>Show Deleted Only".
  //               "<input type=\"checkbox\" name=\"searchisdeleted\" id=\"searchisdeleted\" onchange=\"hideadd()\">"."</label>";
   $hidewidth="width='25'";
 ...
 echo <<< EOF
  ...
  <script language="javascript" type="text/javascript">
    ...
    if(document.getElementById("searchisdeleted").checked){
     ...
     var myCol  = g.getColumnObject(6);
     myCol.setWidth(50);
     g.getColumnObject(4).setWidth(300);
         for (var i=0;i<=4;i++){
         g.getColumnObject(i).setEditable(false);
         }
    }
    else{
     ...
     var myCol  = g.getColumnObject(6);
     myCol.setWidth(0);
     g.getColumnObject(4).setWidth(350);
         for (var i=0;i<=4;i++){
         g.getColumnObject(i).setEditable(true);
         }
    }
    ...
    search();
     ...
     function search(){ 
        ...
        var searchjobposition_no=document.getElementById("searchjobposition_no").value;
        var searchjobposition_name=document.getElementById("searchjobposition_name").value;
	...
	grid.getDataSource().setGetHandlerParameter('searchjobposition_no',searchjobposition_no);
	grid.getDataSource().setGetHandlerParameter('searchjobposition_name',searchjobposition_name);
   ...
   function save()
     {
      ...
      document.getElementById('msgbox').innerHTML="<b style=\"color:red\">Job position code and job position name cannot be null</b><br/>";
...
<ntb:grid id="DataboundGrid"
     ...
     gethandler="jobposition.php?action=search&searchisactive=1"
     savehandler="jobposition.php?action=save"
 ...
 <ntb:columns>
   ...
   <ntb:textcolumn  classname="{\$rh}" width="120" label="Job Position Code" xdatafld="jobposition_no" ></ntb:textcolumn>

   <ntb:textcolumn  classname="{\$rh}" width="297" label="Job Position Name" xdatafld="jobposition_name" ></ntb:textcolumn>
...
EOF;
 ...
 echo <<< EOF
      <ntb:numbercolumn   label="ID"  width="0" xdatafld="jobposition_id" mask="###0" sortenabled="false">
                    </ntb:numbercolumn>
