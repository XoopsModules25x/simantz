...
class Permission {
  ...
  private $tablehrpermission;
  private $tablewindow;
  private $log;

public function Permission (){
	global $xoopsDB,$log,$module_id,$tablehrpermission,$tablegroups,$tablegroups_users_link,$tablewindow,$tablegrouppermission,
	...
	$this->tablehrpermission=$tablehrpermission;
	//$this->tableusers=$tableusers;
	$this->log=$log;
...
/*	$sql="SELECT * FROM (SELECT w.filename, w.window_name, w.window_id,(CASE p.groupid WHEN $groupid THEN $groupid
		...
		LEFT JOIN $this->tablehrpermission p on p.window_id=w.window_id order by w.functiontype, w.window_name)
		as t";*/
	$sql="SELECT w.window_id,w.window_name,w.filename,w.functiontype,gp.groupid,gp.isreadonly FROM $this->tablewindow w
	LEFT JOIN (SELECT groupid,window_id,isreadonly FROM
	$this->tablehrpermission p WHERE p.groupid=$groupid) gp on gp.window_id=w.window_id
    where w.mid = $module_id or w.mid = 0 order by w.functiontype,w.seqno";
	$this->log->showLog(4,"With SQL: $sql");
...
echo <<< EOF
	 <TABLE>
		<TBODY>
		<TR><TH colspan='6' style='text-align: center'>Add/Edit Group Permission</TH></TR>
		<TR><FORM method='POST' action='permission.php' name='frmUpdatePermission'>
			...
			<Th>Function Type</Th>
                        ...
                        <Th>Readonly <A href='#' onclick='selectallview();'><br>[SELECT ALL]</A>&nbsp;<A href='#'
                        onclick='removeallview();'>[REMOVE ALL]</A><br>
                        <INPUT TYPE='submit' value='save' name='submit'></Th>
	...
	echo <<< EOF
		...
		<TR>	<TD class="$rowtype">$j</TD>
			...
			<TD class="$rowtype">$functiontype</TD>
                        ...
                        <TD class="$rowtype"><INPUT type='checkbox' name="isreadonly[$i]" $checked2>
                        </TD>
...
$i++;
  ...
  public function updatePermission(){
	...
	$sqldel="DELETE FROM $this->tablehrpermission where groupid=$this->groupid";
	...
	if($result)
	{
		...
		if($this->lineallow[$i]=='on'){
                    ...
                    if($this->isreadonly[$i]=="on")
                    $isreadonly = 1;
                    else
                    $isreadonly = 0;

			$sqlinsert="INSERT INTO $this->tablehrpermission (window_id,groupid,
				updated,updatedby,created,createdby,isreadonly) VALUES ($winid,$this->groupid,
				'$timestamp',$this->updatedby,'$timestamp',$this->createdby,$isreadonly)";
  ...
  public function generateMenu($uid,$module_id){
	...
	$sql="select distinct(w.window_id) as window_id,
		...
		inner join $this->tablehrpermission gsp on gsp.groupid=g.groupid
	...
	$query=$this->xoopsDB->query($sql);
	$i=0;
        $output .= "<div id='navbarCP'><ul class='menu' id='menu'>
        <li>
        ...
        $outputmaster = "
        <li>
        <a href='' class='menulink'>Master Data</a>
        <ul>";

        $outputtransaction = "
        <li>
        <a href='' class='menulink'>Transaction</a>
        <ul>";

        $outputreport = "
        <li>
        <a href='' class='menulink'>Report</a>
        <ul>";
	...
	while($row=$this->xoopsDB->fetchArray($query)){
		...
		case "M":
                        ...
                        $outputmaster .= "<li><a href='$filename' title='$window_name'>$window_name</a></li>";
		break;
		case "V":
                        $outputreport .= "<li><a href='$filename' title='$window_name'>$window_name</a></li>";

		break;
		default:
                        $outputtransaction .= "<li><a href='$filename' title='$window_name'>$window_name</a></li>";
        ...
        $outputmaster .= "</ul></li>";
        $outputreport .= "</ul></li>";
        $outputtransaction .= "</ul></li>";
        $output .= $outputmaster.$outputtransaction.$outputreport."</ul></div>";
 ...
 public function selectAvailableSysUser($id,$includenull='Y',$ctrlname='uid',$onchangefunction="",$wherestring=""){
	$this->log->showLog(3,"Retrieve available system users from database, with id: $id");
	//$tableusers=$this->tableprefix."users";
	$sql="SELECT uid,uname from sim_users where (uid>0 or uid=$id) $wherestring order by uname";
	...
	return $selectctl;
        ...
        public function checkReadonly($uid,$module_id,$usefilename){
        $retval = 0;
	$this->log->showLog(3, "checkPermission(uid=$uid,module_id=$module_id,usefilename=$usefilename)");
	$sql="select gsp.isreadonly
		from sim_groups g
		inner join $this->tablegroups_users_link gul on g.groupid=gul.groupid
		inner join $this->tableusers u on gul.uid=u.uid
		inner join $this->tablegrouppermission gp on gp.gperm_groupid=g.groupid
