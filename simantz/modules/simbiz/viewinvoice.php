...
$footersummarystartx=80;
   ...
   $reporttitle = array("Unknown","Sales Invoice","Purchase Invoice");
...
function Header()
{
	...
	global   $w0header,$w1header, $header, $headeraccounts_code, $headeraccounts_name,  $ledgertitlefontname,  
		$ledgertitlefontsize,$document_no,$spinvoice_prefix,$document_date,$ref_no,$documenttype,
	...
	$this->Cell(120,$orgnamefontheight,"$orgname",0,0,'R');
...
//	$this->SetXY($marginx[$statementpapersource],$marginy[$statementpapersource]+$orgcontactfontheight * 4);
	...
	$this->Cell(20,$defaultfontheight,"Document No ",0,0,'L');
	$this->Cell(0,$defaultfontheight," : $spinvoice_prefix$document_no",0,0,'L');
...
//	$periodfrom_id=2;
//	$periodto_id=2;	
	$sqlheader="SELECT dcn.*,cur.currency_name FROM $tableinvoice dcn
		INNER JOIN $tablecurrency cur on cur.currency_id=dcn.currency_id
		WHERE dcn.invoice_id=$invoice_id";
	...
	$originalamt=$row['originalamt'];
	  ...
	  $sql="SELECT dcnl.invoiceline_id,dcnl.subject, dcnl.amt,dcnl.qty,dcnl.uom, dcnl.description,dcnl.unitprice
		FROM $tableinvoiceline dcnl
		where dcnl.invoice_id=$invoice_id order by dcnl.invoiceline_id";
	...
	while ($rowline=$xoopsDB->fetchArray($query)){
		$invoiceline_id=$rowline['invoiceline_id'];
