...
class Student
{
  ...
  public function getSqlStrStudentList( $wherestring,  $orderbystring,  $startlimitno ) {
    $sql= "SELECT s.student_id, s.student_code,s.alternate_name, s.student_name, s.isactive, s.dateofbirth,std.standard_name, s.gender, s.ic_no, 
	 ...
	 r.races_name,s.description,s.email,s.web,s.levela,s.levelb,s.levelc,s.religion_id, re.religion_name, p.parents_email
	...
	else
	{
		...
		*/
		if($this->student_id>0){
			$action="update";
			$addressctl="<input type='submit' value='Address' name='submit'  style='height: 40px;'>".
				"<input type='hidden' value='$this->student_id' name='student_id'>".
				"<input type='hidden' value='create' name='action'>";
			
			if($this->allowDelStudent($this->student_id))
			  $deletectrl="<FORM action='labelstudent.php' method='POST' onSubmit='return confirm(" . 
		...
		$addnewctrl="<Form action='labelstudent.php' method='POST'><input name='submit' value='New' type='submit'></form>";
		$namecardctrl="<Form action='printnamecard.php' method='POST' target='_blank'>".
				"<input type='submit' value='Name Card' name='submit'  style='height: 40px;'>".
...
//		$address=$this->ad->showAddress($this->address_id); //$ad->showAddress($this->address_id);
		...
		$jumptotoregclass="<Form action='regclass.php' method='POST'>".
				"<input name='submit' value='Go To Class Registration' style='height: 40px;' type='submit'>".
...
<form action="labelstudent.php" method="post" id="frmStudent" name='frmStudent' onSubmit='return validateStudent()' 
...
<tbody>
	...
	<tr>
		...
		<td class="odd" colspan='3'><input name="description" value="$this->description" size='100' maxlength='100'></td>
	</tr>
	</tbody>
</table>
...
EOF;
//<form action="labelstudent.php" method="post" name="frmShowAllStudent"><input type='submit' value='Show All Students' //name='submit'><input type='hidden' value='showall' name='action'></form>
  ...
  public function showStudentTable( $wherestring="",$orderbystring="",$startlimitno=0,$tabletype='student' ) {
	...
	$sql=$this->getSqlStrStudentList($wherestring,"ORDER BY student_name",0);
...
document.frmchoosestudent.submit.click();
</script>
EOF;
 } //end of showStudentTable


  public function showStudentAddressTable( $wherestring="",$orderbystring="",$startlimitno=0,$tabletype='student' ) {
	$this->log->showLog(3,"Showing Student Table");
	$sql=$this->getSqlStrStudentList($wherestring,"ORDER BY student_name",0);
	
	$query=$this->xoopsDB->query($sql);
	echo <<< EOF
	<form name="frmSearchAddress" action="viewstudentaddress.php" method="POST" target="_BLANK">
	<table style="text-align:left width: 100%" border="1" cellpadding="0" cellspacing="3">
  		<tbody>
    			<tr ><th colspan='9' style='text-align:center'>Student Info</th><th  style='text-align:center' colspan='4'>Parents Info</th></tr><tr>
				<th style="text-align:center;">No</th>
				<th style="text-align:center;">Index No</th>
				<th style="text-align:center;">Student Name</th>
				<th style="text-align:center;">IC No</th>
				...
				<th style="text-align:center;">Select<input type='checkbox' onclick="selectStudent(this.checked)"></th>
   	</tr>
EOF;
	$rowtype="";
	$i=0;
	while ($row=$this->xoopsDB->fetchArray($query)){
		...
		<tr>
			<input type='hidden' value="$student_id" name="studentid_address[$i]">
			<td class="$rowtype" style="text-align:center;">$i</td>
			<td class="$rowtype" style="text-align:center;">$student_code</td>
			<td class="$rowtype" style="text-align:center;"><a href='student.php?action=edit&student_id=$student_id' target="_blank"
					 >$student_name/$alternate_name</a></td>
			...
			<td class="$rowtype" style="text-align:center;"><input type="checkbox" name="isselect[$i]"></td>
...
echo <<< EOF
		...
		<tr height="40" >
		<td align="left" nowrap colspan="10" style="vertical-align:bottom">
		<input style="height:40px;" type="button" name="btnAddress" value="Address List" onclick="printReport(this.value);">
		<input style="height:40px;" type="button" name="btnNamecard" value="Name Card" onclick="printReport(this.value);">
		<input style="height:40px;" type="button" name="btnemail" value="Get Email Address" onclick="printReport(this.value);">
		<input style="height:40px;" type="button" name="btnhp" value="Get HP No" onclick="printReport(this.value);">
		<input style="height:40px;" type="button" name="btnAllData" value="Show Complete Table" onclick="printReport(this.value);">
		</td>
		</tr>
EOF;
...
EOF;
	...
	while ($row=$this->xoopsDB->fetchArray($query)){
		...
		<tr>
			<td class="$rowtype" style="text-align:center;">$i</td>
			<td class="$rowtype" style="text-align:center;">$student_code</td>
			<td class="$rowtype" style="text-align:left;"><a href='labelstudent.php?action=edit&student_id=$student_id' 
			...
			<td class="$rowtype" style="text-align:center;">$gender</td>
			<td class="$rowtype" style="text-align:center;">$isactive</td>
			<td class="$rowtype" style="text-align:center;"><form action='labelstudent.php' method='POST'>
			<input type='image' src='images/edit.gif' name='submit' title='Edit this record'>
...
echo  "</tr></tbody></table>";
  ...
  public function getStudentSelectBox($id,$showEmpty='N'){
  	$sql="SELECT student_id,student_name,student_code from $this->tablestudent where (isactive='Y' or student_id=$id) and student_id>0 order by student_name ;";
 ...
 /**Return a long hyperlink string which contrain a 1st letter of all student name
  ...
  public function searchAToZ(){
	...
	$sqlfilter="SELECT DISTINCT(LEFT(student_name,1)) as shortname FROM $this->tablestudent where isactive='Y' and student_id>0 order by student_name";
...
echo "<b>Student Grouping By Name: </b><br>";
	while ($row=$this->xoopsDB->fetchArray($query)){
		...
		echo "<A href='labelstudent.php?filterstring=$shortname'> $shortname </A> ";
	}
		echo <<<EOF
<!--<BR>
<A href='labelstudent.php' style='color: GRAY'> [ADD NEW STUDENT]</A>
<A href='labelstudent.php?action=search' style='color: gray'> [SEARCH STUDENT]</A>
<A href='labelstudent.php?action=searchaddress' style='color: gray'> [PRINT ADDRESS]</A>-->
...
*/
 ...
 public function showSearchForm($actionname="searchstudent"){
	
	if($actionname == "searchstudent")
   ...
   echo <<< EOF
	...
	<FORM action="labelstudent.php" method="POST">
	  <table style="text-align: left; width: 100%;" border="1" cellpadding="0" cellspacing="3">
	  <tbody>
	    <tr>
	...
	<tr>
	      ...
	      <td class='odd'><input name='joindate'></td>
	...
	<table style="width:150px;">
	  <tbody>
	    <tr>
	      <td><input style="height:40px;" type='submit' value='Search' name='submit'><input type='hidden' name='action' value="$actionname"></td>
