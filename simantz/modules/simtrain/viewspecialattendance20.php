...
$this->SetFont('Times','',10);
  ...
  //  $this->Cell(31,7,"Standard Fees($this->cur_symbol)",1,0,'C');
   ...
   // $this->Cell(19,7,"$this->amt",1,0,'C');
    ...
    $this->SetFont('Times','B',10);
    $this->Cell(20,7,"Special",1,0,'C');
    $this->SetFont('Times','',10);
    $this->Cell(20,7,"Class Count",1,0,'C');
    $this->SetFont('Times','B',10);
    $this->Cell(30,7,$this->classcount,1,0,'C');
    $this->Ln(); 
    $this->SetFont('Times','',10);
...
$i=0;
   ...
   $w=array(6,14,54,20,16,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,36); //12 data
...
while($i<26){//complete show column
...
//	$header=array('No','Code','Student Name', 'Contact No.', 'School', 'Fees', '1st', '2nd', '3rd', '4th', '5th', 'RMK');
	...
	$sql="SELECT s.student_id,s.student_code,s.alternate_name, s.student_name,
		concat(s.hp_no,'/', s.tel_1) as hp_no, sch.school_name, std.standard_name,x.amt, x.isactive, x.studentclass_id".
