...
function Header()
{
   ...
   $this->SetXY(12,6);
    ...
    $this->SetXY(100,6);
    $this->Cell(0,17,"Sales Turn Over Report",0,0,'R');


    $this->SetXY(80,19);
    $this->SetFont('Times','',10);
    $this->Cell(17,8,"Date From",1,0,'L');
    $this->SetFont('Times','B',10);
    $this->Cell(20,8,$this->datefrom,1,0,'C');
    $this->SetFont('Times','',10);
    $this->Cell(17,8,"Date To",1,0,'C');
    $this->SetFont('Times','B',10);
    $this->Cell(20,8,$this->dateto,1,0,'C');
    $this->SetFont('Times','',10);
    $this->Cell(21,8,"Organization",1,0,'L');
    $this->SetFont('Times','B',10);
    while($this->GetStringWidth($this->organization_code)>24)
	$this->organization_code=substr_replace($this->organization_code,"",-1);

    $this->Cell(25,8,$this->organization_code,1,0,'C');
    $this->Ln(10);
$i=0;
...
//	$header=array('No','Student','Type','Code','Date', 'Fees(RM)','Trans.(RM)', 'Paid(RM)', 'Due (RM)','Paid Date','Paid To');
	...
	$this->SetFont('Arial','B',9); 
       	...
       	$this->Cell(40,8,'Student',1,0,'C');
       	$this->Cell(9,8,'Type',1,0,'C');
       	$this->Cell(19,8,'Code',1,0,'C');
       	$this->Cell(16,8,'Date',1,0,'C');
       	$this->Cell(64,4,"Amount in ($this->cur_symbol)",1,0,'C');
       	$this->Cell(17,8,'Paid Date',1,0,'C');
       	$this->Cell(17,8,'Paid To',1,0,'C');
$this->SetXY(102,33);
       	$this->Cell(16,4,'Fees',1,0,'C');
       	$this->Cell(16,4,'Trans',1,0,'C');
       	$this->Cell(16,4,'Paid',1,0,'C');
       	$this->Cell(16,4,'Due',1,0,'C');
...
{
    $w=array(6,40,9,19,16,16,16,16,16,17,17);
	$i=0;
...
$sql=" SELECT sc.studentclass_id, s.student_id,concat(s.alternate_name,'|||', s.student_name) as student_name, sc.tuitionclass_id,sc.movement_id,
		(CASE WHEN tc.tuitionclass_id >0 
		  ...
		  (CASE WHEN tc.tuitionclass_id >0 THEN tc.description
				ELSE concat(pd.product_name,'x',i.quantity*-1) END ) as name, 
