...
function Header()
{
   // $this->Image('./images/simonlogobk.jpg', 11 , 6 , 90 , '' , 'JPG' , 'payment.php');
	...
	$this->setXY(120,14);
	$this->MultiCell(100,4,"$this->jpn_no    $this->rob_no",0,'L');
	...
	$this->setXY(40,25);
	$this->MultiCell(180,5,$this->student_name,0,'L');	
	$this->setXY(120,25);
	$this->SetFont('Times','',12);
	...
	$header=array("Ref. No.","Date","Description""Paid Today($this->cur_symbol)");
...
//    $this->MultiCell(140,3,'Never Live Without Vision',0,'L');
    ...
    $this->SetY(-20);
	$this->MultiCell(145,3,'Total Charge: ',0,'R');
	$this->MultiCell(145,3,'Paid: ',0,'R');
	$this->MultiCell(145,3,'Change: ',0,'R');
//	$this->MultiCell(145,3,'Outstanding Amt: ',0,'R');
	if($this->endreport=='Y'){
		$this->SetFont('Arial','B',8);
		$this->SetY(-20);
		$this->MultiCell(167,3,$this->amt,0,'R');
		$this->MultiCell(167,3, $this->receivedamt,0,'R');
		$this->MultiCell(167,3,$this->returnamt,0,'R');
//		$this->MultiCell(167,3,$this->outstandingamt,0,'R');
	}
...
//$pdf=new PDF('L','mm', 'A5');
	...
	$sql="SELECT student_code, student_name from $tablestudent WHERE student_id=$student_id";
