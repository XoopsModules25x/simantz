...
function Header()
{
   ...
   $this->Image('upload/images/attlistlogo.jpg', 12 , 10 , 60 , '' , 'JPG' , '');
    ...
    $this->SetXY(100,6);
    $this->Cell(0,17,"Transport Fees Report",0,0,'R');
...
//    $this->SetFont('Times','B',9);
	$header=array('No','User','Student','Date/Time', "Class", 
		 'Doc No',"Amount($this->cur_symbol)", "Accum.($this->cur_symbol)","Remark");
    $w=array(8,13,49,26,20,18,18,20,18);
...
/*	$sql="select u.uid,u.uname,s.student_name, s.alternate_name, py.payment_datetime AS payment_datetime,
		 pl.transportamt, py.receipt_no AS docno,o.organization_code, t.tuitionclass_code
		 from $tablepayment py 
		 join $tablepaymentline pl on pl.payment_id = py.payment_id
		 left join $tablestudentclass sc on sc.studentclass_id = pl.studentclass_id
		 join $tabletuitionclass t on sc.tuitionclass_id=t.tuitionclass_id
		 ...
		 where  pl.transportamt <>0 and py.iscomplete = 'Y' and py.organization_id=$organization_id 
		 AND $wherestring 
...
*/
	$sql="select u.uid,u.uname,s.student_name, s.alternate_name, py.payment_datetime AS payment_datetime,
		 ...
		 left join $tablepaymentline pl on sc.studentclass_id = pl.studentclass_id
		 inner join $tablepayment py on pl.payment_id = py.payment_id
		 inner join $tabletuitionclass t on sc.tuitionclass_id=t.tuitionclass_id
		 inner join $tablestudent s on sc.student_id = s.student_id 
		 inner join $tableusers u on py.updatedby = u.uid 
		 inner join $tableorganization o on o.organization_id = py.organization_id 
		 where s.student_id>0 and sc.tuitionclass_id>0 and t.tuitionclass_id>0 and  py.organization_id=$organization_id 
		 AND $wherestring 
		 order by py.payment_datetime desc";
	...
	while($row=$xoopsDB->fetchArray($query)) {
		...
		$docno=$row['docno'];
		$total=$total+$transportamt;
		$data[]=array($i,$uname,$alternate_name."|||".$student_name,$payment_datetime,$tuitionclass_code,
				$docno,$transportamt,number_format($total,2),"");
		//$data[]=array($i,"","","","","","","","","","","","","");
...
<div style="border: 1px solid rgb(153, 153, 255); color: rgb(0, 0, 153);"><big><big><big><span style="font-weight: bold;">Transport Fees Report</span></big></big></big></div><br>
	<table border='1'>
		<tbody>
			<tr><form name='frmincome' action='viewtransportincome.php' method='post' target='_blank'>
