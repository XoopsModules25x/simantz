...
if ($isactive=="Y" or $isactive=="on")
 ...
 switch ($action){
  ...
  case "create" :
	...
	$log->showLog(3,"Accessing create record event, with category name=$o->category_code");
	...
	if($o->insertCategory()){
		 $latest_id=$o->getLatestCategoryID();
			 redirect_header("category.php",$pausetime,"Your data is saved, the new id=$latest_id");
		}
	else {
		$log->showLog(1,"Warning, '$o->category_code' cannot save, please make sure all data is insert correctly.");
		...
		$log->showLog(1,"Warning, '$o->category_code' cannot save due to token expired.");
		$token=$s->createToken($tokenlife,"CREATE_CAT");
		$o->orgctrl=$permission->selectionOrg($o->created,$organization_id);
		$o->getInputForm("new",-1,$token);
		$o->showCategoryTable(); 
...
break;
  ...
  case "edit" :
	if($o->fetchCategory($o->category_id)){
		...
		redirect_header("category.php",3,"<b style='color:red'>Some error on viewing your category data, probably database corrupted.</b>");
  ...
  case "update" :
	if ($s->check(false,$token,"CREATE_CAT")){
		...
		if($o->updateCategory()) //if data save successfully
			...
			redirect_header("category.php?action=edit&category_id=$o->category_id",$pausetime,"<b style='color:red'>Warning! Can't save the data, please make sure all value is insert properly.</b>");
		}
	else{
		redirect_header("category.php?action=edit&category_id=$o->category_id",$pausetime,"<b style='color:red'>Warning! Can't save '$o->category_code' due to token expired.</b>");
	}
  break;
  case "delete" :
	if ($s->check(false,$token,"CREATE_CAT")){
		if($o->deleteCategory($o->category_id))
			...
			redirect_header("category.php?action=edit&category_id=$o->category_id",$pausetime,"<b style='color:red'>Warning! Can't delete data from database due to dependency error.</b>");
	}
	else
		redirect_header("category.php?action=edit&category_id=$o->category_id",$pausetime,"<b style='color:red'>Warning! Can't delete data from database due to token expired!</b>");
