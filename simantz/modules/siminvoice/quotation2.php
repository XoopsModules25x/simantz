   ...
   return IsNumber;
	...
	var customer_id = document.forms['frmQuotation'].customer_id.value;
  	...
  	while(i< document.forms['frmQuotation'].elements.length){
		...
		/*
		if(ctlname=="item_name[]"){
			if(data==""&&document.forms['frmQuotation'].elements[i-1].value==0){
			alert (ctlname + ":" + data + ":isempty, please insert value!");
			document.forms['frmQuotation'].elements[i].style.backgroundColor = "#FF0000";
			document.forms['frmQuotation'].elements[i].focus();
			return false;
			}
	...
	while(i< document.forms['frmQuotation'].elements.length){
		...
		if(j==line){
			...
			var uprice = document.forms['frmQuotation'].elements[i+1].value;
			var discount = document.forms['frmQuotation'].elements[i+3].value;
		
			document.forms['frmQuotation'].elements[i+4].value = qty*uprice - (discount/100*uprice);
	...
	while(i< document.forms['frmQuotation'].elements.length){
		...
		if(j==line){
			var qty = document.forms['frmQuotation'].elements[i-1].value;
	...
	while(i< document.forms['frmQuotation'].elements.length){
		...
		if(j==line){
			var qty = document.forms['frmQuotation'].elements[i-3].value;
...
//when user press save for change existing organization data
  ...
  $fldSort = '';
	...
	$o->showSearchTable("$wherestr","$sortstr",0,9999,"$orderctrl","$fldSort",$s->createToken(120,"CREATE_INV"));
  ...
  $o->termsctrl=$t->getSelectTerms($o->terms_id);
  //$o->itemctrl=$o->getSelectItemArray(0);
//	$o->quotation_date="0000-00-00";	  
	//$o->quotation_date=$_GET['quotation_date'];
...
//	$o->currencyctrl=$cr->getSelectCurrency(0);
	if ($filterstring=="")
		$filterstring="A";
		$o->showQuotationTable("WHERE a.customer_name LIKE '$filterstring%' and a.customer_id = c.customer_id","order by a.customer_name",0,99999,$s->createToken(120,"CREATE_INV"));
  break;
