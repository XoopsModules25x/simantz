...
{
  public $company_id="Unknown";
  public $worker_no="Unknown";
  public $company_name="Unknown"; 
...
function Header()
{
    ...
    $this->Cell(40,17,"$this->company_no",1,0,'L');
    $this->Cell(0,17,"$this->company_name",1,0,'L');

    $this->SetFont('Times','B',10);
	$this->SetXY(90,4);
    $this->Cell(40,17,"Company No ",0,0,'L');
	$this->SetXY(130,4);
    $this->Cell(0,17,"Company Name",0,0,'L');

    $this->Ln(25);
    $this->SetFont('Arial','B',12);
    $this->Cell(32,8,"Report Title",1,0,'L');
    $this->Cell(0,8,"Customer Employment History",1,0,'C');
...
{
    $w=array(7,20,40,18,18,25,25,8,13,16);
	$i=0;
...
//}
	$sql="SELECT w.worker_id,w.worker_code,w.worker_name,c.company_id,c.company_no,c.company_name, wc.worker_no, wc.position, ".
	...
	$data=array();
	$i=0;
	$data[]=array("No","Worker Code","Worker Name" ,"Join Date", "Resign Date" ,"Department", "Position"
			,"Cur.","Salary","Status");
	while($row=$xoopsDB->fetchArray($query)){
		...
		$pdf->company_name=$row['company_name'];
		$pdf->company_no=$row['company_no'];
   	...
   	$data[]=array($i,$worker_code,$worker_name,$joindate,$resigndate,$department,$position,
