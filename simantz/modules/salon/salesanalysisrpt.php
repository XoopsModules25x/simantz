...
function Header()
{
	...
	$this->Cell(0,4,"Sales Analysis Report",0,1,'L');
	...
	*/
	...
	$header =array("No","Date","Item","Type","Qty","Amount ($this->cur_symbol)");
	$w=array(10,20,61,30,30,35); // total width = 285
...
function BasicTable($header,$data,$printheader)
{
	...
	//$w=array(10,20,61,30,30,35); // total width = 285
	$w=array(10,20,61,30,12,18,35); // total width = 285
...
if ($_POST){
	...
	if($product_id > 0)
	$wherestr = " and b.product_id = $product_id ";
	if($category_id > 0)
	...
	$sql = "select *,sum(b.salesline_amount) as tot_amount, sum(b.salesline_qty) as tot_qty 
		...
		$wherestr 
		group by a.sales_date,b.product_id
	...
	$isitem = "Others";
 	...
 	$data[]=array($i,$date,$product_name,$isitem,$qty,$uom,$amount);
