...
function Header()
{
	...
	$this->Cell(0,4,"Stock Replenish Report",0,1,'L');
...
/*
	...
	*/
	...
	$header =array("No","Code","Product","Safety Qty","System Qty","Short Qty","Unit Price","Amount ","Remarks");
	$w=array(10,15,45,17,23,20,21,15,25); // total width = 285
...
function BasicTable($header,$data,$printheader)
{
	...
	$w=array(10,15,45,17,13,10,20,21,15,25); // total width = 285
	...
	$this->Cell(15,6,$total_amount,1,0,'C');
...
if ($_POST){
	...
	$sql = "select 	pl.product_name as product_name,
			...
			pl.sellingprice as sellingprice,
	...
	$shortqty = $row['safety_level'] - $row['total_qty'];

	if($shortqty <= 0){
	$shortqty = "0";
	$amountshort = "0.00";
	}else{
	$amountshort = $row['sellingprice'] * $shortqty;
	$amountshort = number_format($amountshort, 2, '.','');
	}

	
  	
 	$data[]=array($i,$row['product_no'],$row['product_name'],$row['safety_level'],$row['total_qty'],$row['uom'],$shortqty,$row['sellingprice'],$amountshort,"");
