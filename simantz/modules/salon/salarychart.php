...
function Header()
{
	...
	$this->SetXY(80,5);
	$this->Image('./images/attlistlogo.jpg', 150 , 5 , 50 , '' , 'JPG' , '');
	$this->Ln(12);
	$this->SetFont('Arial','B',13);

	//-------------------------
	
	$this->Cell(0,4,"Performance Summary (Chart)",0,1,'L');
	$this->SetFont('Arial','',6);
	$this->Cell(0,4,"Date : $date Time : $time  Page ".$this->PageNo()." / {nb} ",0,1,'R');
	//$this->Ln();
	$this->Cell(195,0,"",1,1,'L');
	$this->SetX(10);
...
if ($_POST){
	...
	$dataSet->addPoint(new Point($row[0], $row[8]));
	$dataSet2->addPoint(new Point($row[0], $row[9]));
	$dataSet3->addPoint(new Point($row[0], $row[10]));				
	$i++;
	}
	...
	$chart->setTitle("Performance Summary (Sales ($pdf->cur_symbol))");
	$chart->render("chart/chartimage/".$timestamp."1.png"); 

	$chart2->setDataSet($dataSet2);
	$chart2->setTitle("Performance Summary (Commission ($pdf->cur_symbol))");
	$chart2->render("chart/chartimage/".$timestamp."2.png"); 

	$chart3->setDataSet($dataSet3);
	$chart3->setTitle("Performance Summary (Salary ($pdf->cur_symbol))");
	$chart3->render("chart/chartimage/".$timestamp."3.png"); 	
	...
	$pdf->SetXY(10,20);
	$pdf->Image("chart/chartimage/".$timestamp."1.png", 10 , 35 , 190 , '' , 'PNG' , '');

	$pdf->Image("chart/chartimage/".$timestamp."2.png", 10 , 115 , 190 , '' , 'PNG' , '');

	$pdf->Image("chart/chartimage/".$timestamp."3.png", 10 , 190 , 190 , '' , 'PNG' , '');
	//$pdf->BasicTable($header,$data,1);
 
	//$pdf->MultiCell(175,7,$sql,1,'C');
	//display pdf
	$pdf->Output();
	unlink("chart/chartimage/".$timestamp."1.png");
	unlink("chart/chartimage/".$timestamp."2.png");
	unlink("chart/chartimage/".$timestamp."3.png");
	exit (1);
