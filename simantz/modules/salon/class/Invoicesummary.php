<?php
/************************************************************************
Class Invoicesummary.php - Copyright kfhoo
**************************************************************************/
...
class Invoicesummary
{
  ...
  public function Invoicesummary($xoopsDB,$tableprefix,$log){
  ...
  public function insertInvoicesummary( ) {
   ...
   * @access public
   */
  public function updateInvoicesummary() {
    $timestamp= date("y/m/d H:i:s", time()) ;
	$sql="";
   ...
   * @access public
   */
  public function getSqlStr_AllInvoicesummary( $wherestring,  $orderbystring,  $startlimitno ) {
   ...
   $this->log->showLog(4,"Running Invoicesummary->getSQLStr_AllInvoicesummary: $sql");
  return $sql;
   ...
   * @access public
   */
  public function fetchInvoicesummaryInfo( $expenses_id ) {
    
	$this->log->showLog(3,"Fetching expenseslist detail into class Invoicesummary.php.<br>");
		
	$sql="SELECT * FROM $this->tableexpenses
	where expenses_id=$expenses_id";
	
	$this->log->showLog(4,"Invoicesummary->fetchInvoicesummaryInfo, before execute:" . $sql . "<br>");
	...
	if($row=$this->xoopsDB->fetchArray($query)){
		...
		$this->isactive=$row['isactive'];
	   	...
	   	$this->log->showLog(4,"Invoicesummary->fetchInvoicesummaryInfo,database fetch into class successfully");
		return true;
	}
	else{
		return false;
	$this->log->showLog(4,"Invoicesummary->fetchInvoicesummaryInfo,failed to fetch data into databases.");	
	}
  } // end of member function fetchInvoicesummaryInfo
  ...
  public function getInputForm( $type,  $expenses_id,$token ) {
	...
	else
	{
		...
		"<input name='title' value='Invoicesummary' type='hidden'>".
		"<input name='btnView' value='View Record Info' type='submit'>".
		"</form>";
		


		$header="Edit Invoicesummary";
...
<table style="width:300px;"><td><form onsubmit="return validateInvoicesummary()" method="post"
 	action="expenses.php" name="frmInvoicesummary"  enctype="multipart/form-data"><input name="reset" value="Reset" type="reset" style='height:40px;'>
  ...
  <tbody>
    ...
    <tr>
		...
		<td class="head"><div $stylecomplete>Complete</div></td>
      	<td class="even"><input type="checkbox" $checked2 name="iscomplete" onclick="completeInvoicesummary(this.checked)" $stylecomplete></td>
    </tr>
...
<tr $displayadd>
	<td class="head">List Invoicesummary :</td>
	<td class="odd">
...
//	$employeectrl = $this->getEmployeeListArray($expensesline_discount,$i);
	$expensesctrl = $this->getSelectInvoicesummarylist($expenseslist_id,$i);
...
EOF;
	...
	if($i == 0){
	echo "<tr><td colspan='8'>Please Add Invoicesummary Line.</td></tr>";
	}
...
EOF;
   ...
   * @access public
   */
  public function deleteInvoicesummary( $expenseslistmaster_id ) {
	...
	if (!rs){
		...
		$this->log->showLog(3,"Invoicesummary ($expenses_id) removed from database successfully!");
		return true;
		
	}
  } // end of member function deleteInvoicesummaryMaster
...
/**
   ...
   * @access public
   */
public function showInvoicesummaryTable(){
	if($this->start_date=="")
	...
	$this->log->showLog(3,"Showing Invoicesummary Table");
	$sql=$this->getSQLStr_AllInvoicesummary($wherestring," GROUP BY a.expenses_id ORDER BY expenses_date desc ",0);
	...
	<form action="invoicesummaryrpt.php" method="POST" name="frmSearch" onsubmit="return validateRpt();" target="_BLANK">
	...
	<td class="even" colspan="3">$this->vendorctrl</td>
	</tr>
...
/**
   ...
   * @access public
   */
  public function getLatestInvoicesummaryID(){
...
public function getSelectInvoicesummary($id,$isitem='NY',$calledfunction="") {
  ...
  public function getInvoicesummaryPrice($id){
...
/*
  ...
  public function getSelectInvoicesummarylist($id,$i) {
  ...
  public function getPriceInvoicesummarylist($id){
	$price = "0.00";
