<?php
/************************************************************************
Class Profitnloss.php - Copyright kfhoo
**************************************************************************/
...
class Profitnloss
{
  ...
  public function Profitnloss($xoopsDB,$tableprefix,$log){
  ...
  public function insertProfitnloss( ) {
   ...
   * @access public
   */
  public function updateProfitnloss() {
    $timestamp= date("y/m/d H:i:s", time()) ;
	$sql="";
   ...
   * @access public
   */
  public function getSqlStr_AllProfitnloss( $wherestring,  $orderbystring,  $startlimitno ) {
   ...
   $this->log->showLog(4,"Running Profitnloss->getSQLStr_AllProfitnloss: $sql");
  return $sql;
   ...
   * @access public
   */
  public function fetchProfitnlossInfo( $expenses_id ) {
    
	$this->log->showLog(3,"Fetching expenseslist detail into class Profitnloss.php.<br>");
		
	$sql="SELECT * FROM $this->tableexpenses
	where expenses_id=$expenses_id";
	
	$this->log->showLog(4,"Profitnloss->fetchProfitnlossInfo, before execute:" . $sql . "<br>");
	...
	if($row=$this->xoopsDB->fetchArray($query)){
		...
		$this->isactive=$row['isactive'];
	   	...
	   	$this->log->showLog(4,"Profitnloss->fetchProfitnlossInfo,database fetch into class successfully");
		return true;
	}
	else{
		return false;
	$this->log->showLog(4,"Profitnloss->fetchProfitnlossInfo,failed to fetch data into databases.");	
	}
  } // end of member function fetchProfitnlossInfo
  ...
  public function getInputForm( $type,  $expenses_id,$token ) {
	...
	else
	{
		...
		"<input name='title' value='Profitnloss' type='hidden'>".
		"<input name='btnView' value='View Record Info' type='submit'>".
		"</form>";
		


		$header="Edit Profitnloss";
...
<table style="width:300px;"><td><form onsubmit="return validateProfitnloss()" method="post"
 	action="expenses.php" name="frmProfitnloss"  enctype="multipart/form-data"><input name="reset" value="Reset" type="reset" style='height:40px;'>
  ...
  <tbody>
    ...
    <tr>
		...
		<td class="head"><div $stylecomplete>Complete</div></td>
      	<td class="even"><input type="checkbox" $checked2 name="iscomplete" onclick="completeProfitnloss(this.checked)" $stylecomplete></td>
    </tr>
...
<tr $displayadd>
	<td class="head">List Profitnloss :</td>
	<td class="odd">
...
//	$employeectrl = $this->getEmployeeListArray($expensesline_discount,$i);
	$expensesctrl = $this->getSelectProfitnlosslist($expenseslist_id,$i);
...
EOF;
	...
	if($i == 0){
	echo "<tr><td colspan='8'>Please Add Profitnloss Line.</td></tr>";
	}
...
EOF;
   ...
   * @access public
   */
  public function deleteProfitnloss( $expenseslistmaster_id ) {
	...
	if (!rs){
		...
		$this->log->showLog(3,"Profitnloss ($expenses_id) removed from database successfully!");
		return true;
		
	}
  } // end of member function deleteProfitnlossMaster
...
/**
   ...
   * @access public
   */
public function showProfitnlossTable(){
	if($this->start_date=="")
	...
	$this->log->showLog(3,"Showing Profitnloss Table");
	$sql=$this->getSQLStr_AllProfitnloss($wherestring," GROUP BY a.expenses_id ORDER BY expenses_date desc ",0);
	...
	<form action="profitlossrpt.php" method="POST" name="frmSearch" onsubmit="return validateRpt();" target="_BLANK">
	...
	<tr>
	<td class="head" nowrap width="15%">Date (YYYY-MM-DD) </td>
...
/**
   ...
   * @access public
   */
  public function getLatestProfitnlossID(){
...
public function getSelectProfitnloss($id,$isitem='NY',$calledfunction="") {
  ...
  public function getProfitnlossPrice($id){
...
/*
  ...
  public function getSelectProfitnlosslist($id,$i) {
  ...
  public function getPriceProfitnlosslist($id){
	$price = "0.00";
