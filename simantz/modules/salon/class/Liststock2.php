<?php
/************************************************************************
Class Liststock.php - Copyright kfhoo
**************************************************************************/
...
class Liststock
{
  ...
  public function Liststock($xoopsDB,$tableprefix,$log){
  ...
  public function insertListstock( ) {
   ...
   * @access public
   */
  public function updateListstock() {
    $timestamp= date("y/m/d H:i:s", time()) ;
	$sql="";
   ...
   * @access public
   */
  public function getSqlStr_AllListstock( $wherestring,  $orderbystring,  $startlimitno ) {
   ...
   $this->log->showLog(4,"Running Liststock->getSQLStr_AllListstock: $sql");
  return $sql;
   ...
   * @access public
   */
  public function fetchListstockInfo( $expenses_id ) {
    
	$this->log->showLog(3,"Fetching expenseslist detail into class Liststock.php.<br>");
		
	$sql="SELECT * FROM $this->tableexpenses
	where expenses_id=$expenses_id";
	
	$this->log->showLog(4,"Liststock->fetchListstockInfo, before execute:" . $sql . "<br>");
	...
	if($row=$this->xoopsDB->fetchArray($query)){
		...
		$this->isactive=$row['isactive'];
	   	...
	   	$this->log->showLog(4,"Liststock->fetchListstockInfo,database fetch into class successfully");
		return true;
	}
	else{
		return false;
	$this->log->showLog(4,"Liststock->fetchListstockInfo,failed to fetch data into databases.");	
	}
  } // end of member function fetchListstockInfo
  ...
  public function getInputForm( $type,  $expenses_id,$token ) {
	...
	else
	{
		...
		"<input name='title' value='Liststock' type='hidden'>".
		"<input name='btnView' value='View Record Info' type='submit'>".
		"</form>";
		


		$header="Edit Liststock";
...
<table style="width:300px;"><td><form onsubmit="return validateListstock()" method="post"
 	action="expenses.php" name="frmListstock"  enctype="multipart/form-data"><input name="reset" value="Reset" type="reset" style='height:40px;'>
  ...
  <tbody>
    ...
    <tr>
		...
		<td class="head"><div $stylecomplete>Complete</div></td>
      	<td class="even"><input type="checkbox" $checked2 name="iscomplete" onclick="completeListstock(this.checked)" $stylecomplete></td>
    </tr>
...
<tr $displayadd>
	<td class="head">List Liststock :</td>
	<td class="odd">
...
//	$employeectrl = $this->getEmployeeListArray($expensesline_discount,$i);
	$expensesctrl = $this->getSelectListstocklist($expenseslist_id,$i);
...
EOF;
	...
	if($i == 0){
	echo "<tr><td colspan='8'>Please Add Liststock Line.</td></tr>";
	}
...
EOF;
   ...
   * @access public
   */
  public function deleteListstock( $expenseslistmaster_id ) {
	...
	if (!rs){
		...
		$this->log->showLog(3,"Liststock ($expenses_id) removed from database successfully!");
		return true;
		
	}
  } // end of member function deleteListstockMaster
...
/**
   ...
   * @access public
   */
public function showListstockTable(){
	if($this->start_date=="")
	...
	$this->log->showLog(3,"Showing Liststock Table");
	$sql=$this->getSQLStr_AllListstock($wherestring," GROUP BY a.expenses_id ORDER BY expenses_date desc ",0);
	...
	<form action="liststockrpt.php" method="POST" name="frmSearch" onsubmit="return validateRpt();" target="_BLANK">
...
/**
   ...
   * @access public
   */
  public function getLatestListstockID(){
...
public function getSelectListstock($id,$isitem='NY',$calledfunction="") {
  ...
  public function getListstockPrice($id){
...
/*
  ...
  public function getSelectListstocklist($id,$i) {
  ...
  public function getPriceListstocklist($id){
	$price = "0.00";
