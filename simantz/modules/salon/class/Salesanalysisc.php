<?php
/************************************************************************
Class Salesanalysisc.php - Copyright kfhoo
**************************************************************************/

class Salesanalysisc
{
  public $expenses_id;
  public $expenses_no;
  ...
  public function Salesanalysisc($xoopsDB,$tableprefix,$log){
  ...
  public function insertSalesanalysisc( ) {
   ...
   * @access public
   */
  public function updateSalesanalysisc() {
    $timestamp= date("y/m/d H:i:s", time()) ;
	$sql="";
   ...
   * @access public
   */
  public function getSqlStr_AllSalesanalysisc( $wherestring,  $orderbystring,  $startlimitno ) {
   ...
   $this->log->showLog(4,"Running Salesanalysisc->getSQLStr_AllSalesanalysisc: $sql");
  return $sql;
   ...
   * @access public
   */
  public function fetchSalesanalysiscInfo( $expenses_id ) {
    
	$this->log->showLog(3,"Fetching expenseslist detail into class Salesanalysisc.php.<br>");
		
	$sql="SELECT * FROM $this->tableexpenses
	where expenses_id=$expenses_id";
	
	$this->log->showLog(4,"Salesanalysisc->fetchSalesanalysiscInfo, before execute:" . $sql . "<br>");
	...
	if($row=$this->xoopsDB->fetchArray($query)){
		...
		$this->isactive=$row['isactive'];
	   	...
	   	$this->log->showLog(4,"Salesanalysisc->fetchSalesanalysiscInfo,database fetch into class successfully");
		return true;
	}
	else{
		return false;
	$this->log->showLog(4,"Salesanalysisc->fetchSalesanalysiscInfo,failed to fetch data into databases.");	
	}
  } // end of member function fetchSalesanalysiscInfo
  ...
  public function getInputForm( $type,  $expenses_id,$token ) {
	...
	else
	{
		...
		"<input name='title' value='Salesanalysisc' type='hidden'>".
		"<input name='btnView' value='View Record Info' type='submit'>".
		"</form>";
		


		$header="Edit Salesanalysisc";
...
<table style="width:300px;"><td><form onsubmit="return validateSalesanalysisc()" method="post"
 	action="expenses.php" name="frmSalesanalysisc"  enctype="multipart/form-data"><input name="reset" value="Reset" type="reset" style='height:40px;'>
  ...
  <tbody>
    ...
    <tr>
		...
		<td class="head"><div $stylecomplete>Complete</div></td>
      	<td class="even"><input type="checkbox" $checked2 name="iscomplete" onclick="completeSalesanalysisc(this.checked)" $stylecomplete></td>
    </tr>
...
<tr $displayadd>
	<td class="head">List Salesanalysisc :</td>
	<td class="odd">
...
//	$employeectrl = $this->getEmployeeListArray($expensesline_discount,$i);
	$expensesctrl = $this->getSelectSalesanalysisclist($expenseslist_id,$i);
...
EOF;
	...
	if($i == 0){
	echo "<tr><td colspan='8'>Please Add Salesanalysisc Line.</td></tr>";
	}
...
EOF;
   ...
   * @access public
   */
  public function deleteSalesanalysisc( $expenseslistmaster_id ) {
	...
	if (!rs){
		...
		$this->log->showLog(3,"Salesanalysisc ($expenses_id) removed from database successfully!");
		return true;
		
	}
  } // end of member function deleteSalesanalysiscMaster
...
/**
   ...
   * @access public
   */
public function showSalesanalysiscTable(){
	if($this->start_date=="")
	...
	$this->log->showLog(3,"Showing Salesanalysisc Table");
	$sql=$this->getSQLStr_AllSalesanalysisc($wherestring," GROUP BY a.expenses_id ORDER BY expenses_date desc ",0);
	...
	<form action="salesanalysischart.php" method="POST" name="frmSearch" onsubmit="return validateRpt();" target="_BLANK">
	...
	<td class="even" colspan="3">$this->categoryctrl</td>
	</tr>
	...
	<tr>
	<td class="head" colspan="4"><input type="submit" value="View Chart"  style='height:40px;'></td>
	</tr>
...
/**
   ...
   * @access public
   */
  public function getLatestSalesanalysiscID(){
...
public function getSelectSalesanalysisc($id,$isitem='NY',$calledfunction="") {
  ...
  public function getSalesanalysiscPrice($id){
...
/*
  ...
  public function getSelectSalesanalysisclist($id,$i) {
  ...
  public function getPriceSalesanalysisclist($id){
	$price = "0.00";
