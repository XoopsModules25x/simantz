<?php
/************************************************************************
Class Salesanalysis.php - Copyright kfhoo
**************************************************************************/
...
class Salesanalysis
{
  ...
  public function Salesanalysis($xoopsDB,$tableprefix,$log){
  ...
  public function insertSalesanalysis( ) {
   ...
   * @access public
   */
  public function updateSalesanalysis() {
    $timestamp= date("y/m/d H:i:s", time()) ;
	$sql="";
   ...
   * @access public
   */
  public function getSqlStr_AllSalesanalysis( $wherestring,  $orderbystring,  $startlimitno ) {
   ...
   $this->log->showLog(4,"Running Salesanalysis->getSQLStr_AllSalesanalysis: $sql");
  return $sql;
   ...
   * @access public
   */
  public function fetchSalesanalysisInfo( $expenses_id ) {
    
	$this->log->showLog(3,"Fetching expenseslist detail into class Salesanalysis.php.<br>");
		
	$sql="SELECT * FROM $this->tableexpenses
	where expenses_id=$expenses_id";
	
	$this->log->showLog(4,"Salesanalysis->fetchSalesanalysisInfo, before execute:" . $sql . "<br>");
	...
	if($row=$this->xoopsDB->fetchArray($query)){
		...
		$this->isactive=$row['isactive'];
	   	...
	   	$this->log->showLog(4,"Salesanalysis->fetchSalesanalysisInfo,database fetch into class successfully");
		return true;
	}
	else{
		return false;
	$this->log->showLog(4,"Salesanalysis->fetchSalesanalysisInfo,failed to fetch data into databases.");	
	}
  } // end of member function fetchSalesanalysisInfo
  ...
  public function getInputForm( $type,  $expenses_id,$token ) {
	...
	else
	{
		...
		"<input name='title' value='Salesanalysis' type='hidden'>".
		"<input name='btnView' value='View Record Info' type='submit'>".
		"</form>";
		


		$header="Edit Salesanalysis";
...
<table style="width:300px;"><td><form onsubmit="return validateSalesanalysis()" method="post"
 	action="expenses.php" name="frmSalesanalysis"  enctype="multipart/form-data"><input name="reset" value="Reset" type="reset" style='height:40px;'>
  ...
  <tbody>
    ...
    <tr>
		...
		<td class="head"><div $stylecomplete>Complete</div></td>
      	<td class="even"><input type="checkbox" $checked2 name="iscomplete" onclick="completeSalesanalysis(this.checked)" $stylecomplete></td>
    </tr>
...
<tr $displayadd>
	<td class="head">List Salesanalysis :</td>
	<td class="odd">
...
//	$employeectrl = $this->getEmployeeListArray($expensesline_discount,$i);
	$expensesctrl = $this->getSelectSalesanalysislist($expenseslist_id,$i);
...
EOF;
	...
	if($i == 0){
	echo "<tr><td colspan='8'>Please Add Salesanalysis Line.</td></tr>";
	}
...
EOF;
   ...
   * @access public
   */
  public function deleteSalesanalysis( $expenseslistmaster_id ) {
	...
	if (!rs){
		...
		$this->log->showLog(3,"Salesanalysis ($expenses_id) removed from database successfully!");
		return true;
		
	}
  } // end of member function deleteSalesanalysisMaster
...
/**
   ...
   * @access public
   */
public function showSalesanalysisTable(){
	if($this->start_date=="")
	...
	$this->log->showLog(3,"Showing Salesanalysis Table");
	$sql=$this->getSQLStr_AllSalesanalysis($wherestring," GROUP BY a.expenses_id ORDER BY expenses_date desc ",0);
	...
	<form action="salesanalysisrpt.php" method="POST" name="frmSearch" onsubmit="return validateRpt();" target="_BLANK">
...
/**
   ...
   * @access public
   */
  public function getLatestSalesanalysisID(){
...
public function getSelectSalesanalysis($id,$isitem='NY',$calledfunction="") {
  ...
  public function getSalesanalysisPrice($id){
...
/*
  ...
  public function getSelectSalesanalysislist($id,$i) {
  ...
  public function getPriceSalesanalysislist($id){
	$price = "0.00";
