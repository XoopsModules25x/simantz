<?php
/************************************************************************
Class Stocksummary.php - Copyright kfhoo
**************************************************************************/
...
class Stocksummary
{
  ...
  public function Stocksummary($xoopsDB,$tableprefix,$log){
  ...
  public function insertStocksummary( ) {
   ...
   * @access public
   */
  public function updateStocksummary() {
    $timestamp= date("y/m/d H:i:s", time()) ;
	$sql="";
   ...
   * @access public
   */
  public function getSqlStr_AllStocksummary( $wherestring,  $orderbystring,  $startlimitno ) {
   ...
   $this->log->showLog(4,"Running Stocksummary->getSQLStr_AllStocksummary: $sql");
  return $sql;
   ...
   * @access public
   */
  public function fetchStocksummaryInfo( $expenses_id ) {
    
	$this->log->showLog(3,"Fetching expenseslist detail into class Stocksummary.php.<br>");
		
	$sql="SELECT * FROM $this->tableexpenses
	where expenses_id=$expenses_id";
	
	$this->log->showLog(4,"Stocksummary->fetchStocksummaryInfo, before execute:" . $sql . "<br>");
	...
	if($row=$this->xoopsDB->fetchArray($query)){
		...
		$this->isactive=$row['isactive'];
	   	...
	   	$this->log->showLog(4,"Stocksummary->fetchStocksummaryInfo,database fetch into class successfully");
		return true;
	}
	else{
		return false;
	$this->log->showLog(4,"Stocksummary->fetchStocksummaryInfo,failed to fetch data into databases.");	
	}
  } // end of member function fetchStocksummaryInfo
  ...
  public function getInputForm( $type,  $expenses_id,$token ) {
	...
	else
	{
		...
		"<input name='title' value='Stocksummary' type='hidden'>".
		"<input name='btnView' value='View Record Info' type='submit'>".
		"</form>";
		


		$header="Edit Stocksummary";
...
<table style="width:300px;"><td><form onsubmit="return validateStocksummary()" method="post"
 	action="expenses.php" name="frmStocksummary"  enctype="multipart/form-data"><input name="reset" value="Reset" type="reset" style='height:40px;'>
  ...
  <tbody>
    ...
    <tr>
		...
		<td class="head"><div $stylecomplete>Complete</div></td>
      	<td class="even"><input type="checkbox" $checked2 name="iscomplete" onclick="completeStocksummary(this.checked)" $stylecomplete></td>
    </tr>
...
<tr $displayadd>
	<td class="head">List Stocksummary :</td>
	<td class="odd">
...
//	$employeectrl = $this->getEmployeeListArray($expensesline_discount,$i);
	$expensesctrl = $this->getSelectStocksummarylist($expenseslist_id,$i);
...
EOF;
	...
	if($i == 0){
	echo "<tr><td colspan='8'>Please Add Stocksummary Line.</td></tr>";
	}
...
EOF;
   ...
   * @access public
   */
  public function deleteStocksummary( $expenseslistmaster_id ) {
	...
	if (!rs){
		...
		$this->log->showLog(3,"Stocksummary ($expenses_id) removed from database successfully!");
		return true;
		
	}
  } // end of member function deleteStocksummaryMaster
...
/**
   ...
   * @access public
   */
public function showStocksummaryTable(){
	if($this->start_date=="")
	...
	$this->log->showLog(3,"Showing Stocksummary Table");
	$sql=$this->getSQLStr_AllStocksummary($wherestring," GROUP BY a.expenses_id ORDER BY expenses_date desc ",0);
	...
	<form action="stocksummaryrpt.php" method="POST" name="frmSearch" onsubmit="return validateRpt();" target="_BLANK">
...
/**
   ...
   * @access public
   */
  public function getLatestStocksummaryID(){
...
public function getSelectStocksummary($id,$isitem='NY',$calledfunction="") {
  ...
  public function getStocksummaryPrice($id){
...
/*
  ...
  public function getSelectStocksummarylist($id,$i) {
  ...
  public function getPriceStocksummarylist($id){
	$price = "0.00";
