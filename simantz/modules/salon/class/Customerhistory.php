<?php
/************************************************************************
Class Customerhistory.php - Copyright kfhoo
**************************************************************************/
...
class Customerhistory
{
  ...
  public function Customerhistory($xoopsDB,$tableprefix,$log){
  ...
  public function insertCustomerhistory( ) {
   ...
   * @access public
   */
  public function getSqlStr_AllCustomerhistory( $wherestring,  $orderbystring,  $startlimitno ) {
   ...
   $this->log->showLog(4,"Running Customerhistory->getSQLStr_AllCustomerhistory: $sql");
  return $sql;
   ...
   * @access public
   */
  public function fetchCustomerhistoryInfo( $expenses_id ) {
    
	$this->log->showLog(3,"Fetching expenseslist detail into class Customerhistory.php.<br>");
		
	$sql="SELECT * FROM $this->tableexpenses
	where expenses_id=$expenses_id";
	
	$this->log->showLog(4,"Customerhistory->fetchCustomerhistoryInfo, before execute:" . $sql . "<br>");
	...
	if($row=$this->xoopsDB->fetchArray($query)){
		...
		$this->isactive=$row['isactive'];
	   	...
	   	$this->log->showLog(4,"Customerhistory->fetchCustomerhistoryInfo,database fetch into class successfully");
		return true;
	}
	else{
		return false;
	$this->log->showLog(4,"Customerhistory->fetchCustomerhistoryInfo,failed to fetch data into databases.");	
	}
  } // end of member function fetchCustomerhistoryInfo
  ...
  public function getInputForm( $type,  $expenses_id,$token ) {
	...
	else
	{
		...
		"<input name='title' value='Customerhistory' type='hidden'>".
		"<input name='btnView' value='View Record Info' type='submit'>".
		"</form>";
		


		$header="Edit Customerhistory";
...
<table style="width:300px;"><td><form onsubmit="return validateCustomerhistory()" method="post"
 	action="expenses.php" name="frmCustomerhistory"  enctype="multipart/form-data"><input name="reset" value="Reset" type="reset" style='height:40px;'>
  ...
  <tbody>
    ...
    <tr>
		...
		<td class="head"><div $stylecomplete>Complete</div></td>
      	<td class="even"><input type="checkbox" $checked2 name="iscomplete" onclick="completeCustomerhistory(this.checked)" $stylecomplete></td>
    </tr>
...
<tr $displayadd>
	<td class="head">List Customerhistory :</td>
	<td class="odd">
...
//	$employeectrl = $this->getEmployeeListArray($expensesline_discount,$i);
	$expensesctrl = $this->getSelectCustomerhistorylist($expenseslist_id,$i);
...
EOF;
	...
	if($i == 0){
	echo "<tr><td colspan='8'>Please Add Customerhistory Line.</td></tr>";
	}
...
EOF;
   ...
   * @access public
   */
  public function deleteCustomerhistory( $expenseslistmaster_id ) {
	...
	if (!rs){
		...
		$this->log->showLog(3,"Customerhistory ($expenses_id) removed from database successfully!");
		return true;
		
	}
  } // end of member function deleteCustomerhistoryMaster
...
/**
   ...
   * @access public
   */
public function showCustomerhistoryTable(){
	if($this->start_date=="")
	...
	$this->log->showLog(3,"Showing Customerhistory Table");
	$sql=$this->getSQLStr_AllCustomerhistory($wherestring," GROUP BY a.expenses_id ORDER BY expenses_date desc ",0);
	...
	<form action="customerhistoryrpt.php" method="POST" name="frmSearch" onsubmit="return validateRpt();" target="_BLANK">
	...
	<td class="head" width="10%">Filter By</td>
	<td class="even" colspan="3" width="90%">
	<select name="filter">
	<option value=""></option>
	...
	<tr id="dateID">
	<td class="head" width="15%" nowrap>Date (YYYY-MM-DD)</td>
	<td class="even" colspan="3" width="85%">
	...
	<tr id="stylistID" astyle="display:none">
	<td class="head">Customer</td>
	<td class="even" colspan="3">$this->customerctrl</td>
	</tr>
...
/**
   ...
   * @access public
   */
  public function getLatestCustomerhistoryID(){
...
public function getSelectCustomerhistory($id,$isitem='NY',$calledfunction="") {
  ...
  public function getCustomerhistoryPrice($id){
...
/*
  ...
  public function getSelectCustomerhistorylist($id,$i) {
  ...
  public function getPriceCustomerhistorylist($id){
	$price = "0.00";
