<?php
/************************************************************************
Class Typecustomer.php - Copyright kfhoo
**************************************************************************/
...
class Typecustomer
{
  ...
  public function Typecustomer($xoopsDB,$tableprefix,$log){
  ...
  public function insertTypecustomer( ) {
   ...
   * @access public
   */
  public function updateTypecustomer() {
    $timestamp= date("y/m/d H:i:s", time()) ;
	$sql="";
   ...
   * @access public
   */
  public function getSqlStr_AllTypecustomer( $wherestring,  $orderbystring,  $startlimitno ) {
   ...
   $this->log->showLog(4,"Running Typecustomer->getSQLStr_AllTypecustomer: $sql");
  return $sql;
   ...
   * @access public
   */
  public function fetchTypecustomerInfo( $expenses_id ) {
    
	$this->log->showLog(3,"Fetching expenseslist detail into class Typecustomer.php.<br>");
		
	$sql="SELECT * FROM $this->tableexpenses
	where expenses_id=$expenses_id";
	
	$this->log->showLog(4,"Typecustomer->fetchTypecustomerInfo, before execute:" . $sql . "<br>");
	...
	if($row=$this->xoopsDB->fetchArray($query)){
		...
		$this->isactive=$row['isactive'];
	   	...
	   	$this->log->showLog(4,"Typecustomer->fetchTypecustomerInfo,database fetch into class successfully");
		return true;
	}
	else{
		return false;
	$this->log->showLog(4,"Typecustomer->fetchTypecustomerInfo,failed to fetch data into databases.");	
	}
  } // end of member function fetchTypecustomerInfo
  ...
  public function getInputForm( $type,  $expenses_id,$token ) {
	...
	else
	{
		...
		"<input name='title' value='Typecustomer' type='hidden'>".
		"<input name='btnView' value='View Record Info' type='submit'>".
		"</form>";
		


		$header="Edit Typecustomer";
...
<table style="width:300px;"><td><form onsubmit="return validateTypecustomer()" method="post"
 	action="expenses.php" name="frmTypecustomer"  enctype="multipart/form-data"><input name="reset" value="Reset" type="reset" style='height:40px;'>
  ...
  <tbody>
    ...
    <tr>
		...
		<td class="head"><div $stylecomplete>Complete</div></td>
      	<td class="even"><input type="checkbox" $checked2 name="iscomplete" onclick="completeTypecustomer(this.checked)" $stylecomplete></td>
    </tr>
...
<tr $displayadd>
	<td class="head">List Typecustomer :</td>
	<td class="odd">
...
//	$employeectrl = $this->getEmployeeListArray($expensesline_discount,$i);
	$expensesctrl = $this->getSelectTypecustomerlist($expenseslist_id,$i);
...
EOF;
	...
	if($i == 0){
	echo "<tr><td colspan='8'>Please Add Typecustomer Line.</td></tr>";
	}
...
EOF;
   ...
   * @access public
   */
  public function deleteTypecustomer( $expenseslistmaster_id ) {
	...
	if (!rs){
		...
		$this->log->showLog(3,"Typecustomer ($expenses_id) removed from database successfully!");
		return true;
		
	}
  } // end of member function deleteTypecustomerMaster
...
/**
   ...
   * @access public
   */
public function showTypecustomerTable(){
	if($this->start_date=="")
	...
	$this->log->showLog(3,"Showing Typecustomer Table");
	$sql=$this->getSQLStr_AllTypecustomer($wherestring," GROUP BY a.expenses_id ORDER BY expenses_date desc ",0);
	...
	<form action="typecustomerrpt.php" method="POST" name="frmSearch" onsubmit="return validateRpt();" target="_BLANK">
	...
	<tr id="dateID">
	<td class="head" width="15%" nowrap>Join Date (YYYY-MM-DD)</td>
	...
	<td class="even" colspan="3">$this->customertypectrl</td>
	</tr>
...
/**
   ...
   * @access public
   */
  public function getLatestTypecustomerID(){
...
public function getSelectTypecustomer($id,$isitem='NY',$calledfunction="") {
  ...
  public function getTypecustomerPrice($id){
...
/*
  ...
  public function getSelectTypecustomerlist($id,$i) {
  ...
  public function getPriceTypecustomerlist($id){
	$price = "0.00";
