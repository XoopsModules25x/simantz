<?php
/************************************************************************
Class Expensessummary.php - Copyright kfhoo
**************************************************************************/
...
class Expensessummary
{
  ...
  public function Expensessummary($xoopsDB,$tableprefix,$log){
  ...
  public function insertExpensessummary( ) {
   ...
   * @access public
   */
  public function updateExpensessummary() {
    $timestamp= date("y/m/d H:i:s", time()) ;
	$sql="";
   ...
   * @access public
   */
  public function getSqlStr_AllExpensessummary( $wherestring,  $orderbystring,  $startlimitno ) {
   ...
   $this->log->showLog(4,"Running Expensessummary->getSQLStr_AllExpensessummary: $sql");
  return $sql;
   ...
   * @access public
   */
  public function fetchExpensessummaryInfo( $expenses_id ) {
    
	$this->log->showLog(3,"Fetching expenseslist detail into class Expensessummary.php.<br>");
		
	$sql="SELECT * FROM $this->tableexpenses
	where expenses_id=$expenses_id";
	
	$this->log->showLog(4,"Expensessummary->fetchExpensessummaryInfo, before execute:" . $sql . "<br>");
	...
	if($row=$this->xoopsDB->fetchArray($query)){
		...
		$this->isactive=$row['isactive'];
	   	...
	   	$this->log->showLog(4,"Expensessummary->fetchExpensessummaryInfo,database fetch into class successfully");
		return true;
	}
	else{
		return false;
	$this->log->showLog(4,"Expensessummary->fetchExpensessummaryInfo,failed to fetch data into databases.");	
	}
  } // end of member function fetchExpensessummaryInfo
  ...
  public function getInputForm( $type,  $expenses_id,$token ) {
	...
	else
	{
		...
		"<input name='title' value='Expensessummary' type='hidden'>".
		"<input name='btnView' value='View Record Info' type='submit'>".
		"</form>";
		


		$header="Edit Expensessummary";
...
<table style="width:300px;"><td><form onsubmit="return validateExpensessummary()" method="post"
 	action="expenses.php" name="frmExpensessummary"  enctype="multipart/form-data"><input name="reset" value="Reset" type="reset" style='height:40px;'>
  ...
  <tbody>
    ...
    <tr>
		...
		<td class="head"><div $stylecomplete>Complete</div></td>
      	<td class="even"><input type="checkbox" $checked2 name="iscomplete" onclick="completeExpensessummary(this.checked)" $stylecomplete></td>
    </tr>
...
<tr $displayadd>
	<td class="head">List Expensessummary :</td>
	<td class="odd">
...
//	$employeectrl = $this->getEmployeeListArray($expensesline_discount,$i);
	$expensesctrl = $this->getSelectExpensessummarylist($expenseslist_id,$i);
...
EOF;
	...
	if($i == 0){
	echo "<tr><td colspan='8'>Please Add Expensessummary Line.</td></tr>";
	}
...
EOF;
   ...
   * @access public
   */
  public function deleteExpensessummary( $expenseslistmaster_id ) {
	...
	if (!rs){
		...
		$this->log->showLog(3,"Expensessummary ($expenses_id) removed from database successfully!");
		return true;
		
	}
  } // end of member function deleteExpensessummaryMaster
...
/**
   ...
   * @access public
   */
public function showExpensessummaryTable(){
	if($this->start_date=="")
	...
	$this->log->showLog(3,"Showing Expensessummary Table");
	$sql=$this->getSQLStr_AllExpensessummary($wherestring," GROUP BY a.expenses_id ORDER BY expenses_date desc ",0);
	...
	<form action="expensessummaryrpt.php" method="POST" name="frmSearch" onsubmit="return validateRpt();" target="_BLANK">
	...
	<td class="head">Expenses Category</td>
...
/**
   ...
   * @access public
   */
  public function getLatestExpensessummaryID(){
...
public function getSelectExpensessummary($id,$isitem='NY',$calledfunction="") {
  ...
  public function getExpensessummaryPrice($id){
...
/*
  ...
  public function getSelectExpensessummarylist($id,$i) {
  ...
  public function getPriceExpensessummarylist($id){
	$price = "0.00";
