<?php
/************************************************************************
Class Salesturnover.php - Copyright kfhoo
**************************************************************************/
...
class Salesturnover
{
  ...
  public function Salesturnover($xoopsDB,$tableprefix,$log){
  ...
  public function insertSalesturnover( ) {
   ...
   * @access public
   */
  public function getSqlStr_AllSalesturnover( $wherestring,  $orderbystring,  $startlimitno ) {
   ...
   $this->log->showLog(4,"Running Salesturnover->getSQLStr_AllSalesturnover: $sql");
  return $sql;
   ...
   * @access public
   */
  public function fetchSalesturnoverInfo( $expenses_id ) {
    
	$this->log->showLog(3,"Fetching expenseslist detail into class Salesturnover.php.<br>");
		
	$sql="SELECT * FROM $this->tableexpenses
	where expenses_id=$expenses_id";
	
	$this->log->showLog(4,"Salesturnover->fetchSalesturnoverInfo, before execute:" . $sql . "<br>");
	...
	if($row=$this->xoopsDB->fetchArray($query)){
		...
		$this->isactive=$row['isactive'];
	   	...
	   	$this->log->showLog(4,"Salesturnover->fetchSalesturnoverInfo,database fetch into class successfully");
		return true;
	}
	else{
		return false;
	$this->log->showLog(4,"Salesturnover->fetchSalesturnoverInfo,failed to fetch data into databases.");	
	}
  } // end of member function fetchSalesturnoverInfo
  ...
  public function getInputForm( $type,  $expenses_id,$token ) {
	...
	else
	{
		...
		"<input name='title' value='Salesturnover' type='hidden'>".
		"<input name='btnView' value='View Record Info' type='submit'>".
		"</form>";
		


		$header="Edit Salesturnover";
...
<table style="width:300px;"><td><form onsubmit="return validateSalesturnover()" method="post"
 	action="expenses.php" name="frmSalesturnover"  enctype="multipart/form-data"><input name="reset" value="Reset" type="reset" style='height:40px;'>
  ...
  <tbody>
    ...
    <tr>
		...
		<td class="head"><div $stylecomplete>Complete</div></td>
      	<td class="even"><input type="checkbox" $checked2 name="iscomplete" onclick="completeSalesturnover(this.checked)" $stylecomplete></td>
    </tr>
...
<tr $displayadd>
	<td class="head">List Salesturnover :</td>
	<td class="odd">
...
//	$employeectrl = $this->getEmployeeListArray($expensesline_discount,$i);
	$expensesctrl = $this->getSelectSalesturnoverlist($expenseslist_id,$i);
...
EOF;
	...
	if($i == 0){
	echo "<tr><td colspan='8'>Please Add Salesturnover Line.</td></tr>";
	}
...
EOF;
   ...
   * @access public
   */
  public function deleteSalesturnover( $expenseslistmaster_id ) {
	...
	if (!rs){
		...
		$this->log->showLog(3,"Salesturnover ($expenses_id) removed from database successfully!");
		return true;
		
	}
  } // end of member function deleteSalesturnoverMaster
...
/**
   ...
   * @access public
   */
public function showSalesturnoverTable(){
	if($this->start_date=="")
	...
	$this->log->showLog(3,"Showing Salesturnover Table");
	$sql=$this->getSQLStr_AllSalesturnover($wherestring," GROUP BY a.expenses_id ORDER BY expenses_date desc ",0);
	...
	<form action="salesturnoverrpt.php" method="POST" name="frmSearch" onsubmit="return validateRpt();" target="_BLANK">
...
/**
   ...
   * @access public
   */
  public function getLatestSalesturnoverID(){
...
public function getSelectSalesturnover($id,$isitem='NY',$calledfunction="") {
  ...
  public function getSalesturnoverPrice($id){
...
/*
  ...
  public function getSelectSalesturnoverlist($id,$i) {
  ...
  public function getPriceSalesturnoverlist($id){
	$price = "0.00";
