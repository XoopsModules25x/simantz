<?php
/************************************************************************
Class Stockmovement.php - Copyright kfhoo
**************************************************************************/
...
class Stockmovement
{
  ...
  public function Stockmovement($xoopsDB,$tableprefix,$log){
   ...
   * @access public
   */
  public function getSqlStr_AllStockmovement( $wherestring,  $orderbystring,  $startlimitno ) {
   ...
   $this->log->showLog(4,"Running Stockmovement->getSQLStr_AllStockmovement: $sql");
  return $sql;
   ...
   * @access public
   */
  public function fetchStockmovementInfo( $expenses_id ) {
    
	$this->log->showLog(3,"Fetching expenseslist detail into class Stockmovement.php.<br>");
		
	$sql="SELECT * FROM $this->tableexpenses
	where expenses_id=$expenses_id";
	
	$this->log->showLog(4,"Stockmovement->fetchStockmovementInfo, before execute:" . $sql . "<br>");
	...
	if($row=$this->xoopsDB->fetchArray($query)){
		...
		$this->isactive=$row['isactive'];
	   	...
	   	$this->log->showLog(4,"Stockmovement->fetchStockmovementInfo,database fetch into class successfully");
		return true;
	}
	else{
		return false;
	$this->log->showLog(4,"Stockmovement->fetchStockmovementInfo,failed to fetch data into databases.");	
	}
  } // end of member function fetchStockmovementInfo
  ...
  public function getInputForm( $type,  $expenses_id,$token ) {
	...
	else
	{
		...
		"<input name='title' value='Stockmovement' type='hidden'>".
		"<input name='btnView' value='View Record Info' type='submit'>".
		"</form>";
		


		$header="Edit Stockmovement";
...
<table style="width:300px;"><td><form onsubmit="return validateStockmovement()" method="post"
 	action="expenses.php" name="frmStockmovement"  enctype="multipart/form-data"><input name="reset" value="Reset" type="reset" style='height:40px;'>
  ...
  <tbody>
    ...
    <tr>
		...
		<td class="head"><div $stylecomplete>Complete</div></td>
      	<td class="even"><input type="checkbox" $checked2 name="iscomplete" onclick="completeStockmovement(this.checked)" $stylecomplete></td>
    </tr>
...
<tr $displayadd>
	<td class="head">List Stockmovement :</td>
	<td class="odd">
...
//	$employeectrl = $this->getEmployeeListArray($expensesline_discount,$i);
	$expensesctrl = $this->getSelectStockmovementlist($expenseslist_id,$i);
...
EOF;
	...
	if($i == 0){
	echo "<tr><td colspan='8'>Please Add Stockmovement Line.</td></tr>";
	}
...
EOF;
   ...
   * @access public
   */
  public function deleteStockmovement( $expenseslistmaster_id ) {
	...
	if (!rs){
		...
		$this->log->showLog(3,"Stockmovement ($expenses_id) removed from database successfully!");
		return true;
		
	}
  } // end of member function deleteStockmovementMaster
...
/**
   ...
   * @access public
   */
public function showStockmovementTable(){
	if($this->start_date=="")
	...
	$this->log->showLog(3,"Showing Stockmovement Table");
	$sql=$this->getSQLStr_AllStockmovement($wherestring," GROUP BY a.expenses_id ORDER BY expenses_date desc ",0);
	...
	<form action="stockmovementrpt.php" method="POST" name="frmSearch" onsubmit="return validateRpt();" target="_BLANK">
...
/**
   ...
   * @access public
   */
  public function getLatestStockmovementID(){
...
public function getSelectStockmovement($id,$isitem='NY',$calledfunction="") {
  ...
  public function getStockmovementPrice($id){
...
/*
  ...
  public function getSelectStockmovementlist($id,$i) {
  ...
  public function getPriceStockmovementlist($id){
	$price = "0.00";
