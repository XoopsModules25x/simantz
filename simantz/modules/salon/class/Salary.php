<?php
/************************************************************************
Class Salary.php - Copyright kfhoo
**************************************************************************/
...
class Salary
{
  ...
  public function Salary($xoopsDB,$tableprefix,$log){
   ...
   * @access public
   */
  public function getSqlStr_AllSalary( $wherestring,  $orderbystring,  $startlimitno ) {
   ...
   $this->log->showLog(4,"Running Salary->getSQLStr_AllSalary: $sql");
  return $sql;
   ...
   * @access public
   */
  public function fetchSalaryInfo( $expenses_id ) {
    
	$this->log->showLog(3,"Fetching expenseslist detail into class Salary.php.<br>");
		
	$sql="SELECT * FROM $this->tableexpenses
	where expenses_id=$expenses_id";
	
	$this->log->showLog(4,"Salary->fetchSalaryInfo, before execute:" . $sql . "<br>");
	...
	if($row=$this->xoopsDB->fetchArray($query)){
		...
		$this->isactive=$row['isactive'];
	   	...
	   	$this->log->showLog(4,"Salary->fetchSalaryInfo,database fetch into class successfully");
		return true;
	}
	else{
		return false;
	$this->log->showLog(4,"Salary->fetchSalaryInfo,failed to fetch data into databases.");	
	}
  } // end of member function fetchSalaryInfo
  ...
  public function getInputForm( $type,  $expenses_id,$token ) {
	...
	else
	{
		...
		"<input name='title' value='Salary' type='hidden'>".
...
<table style="width:300px;"><td><form onsubmit="return validateSalary()" method="post"
 	action="expenses.php" name="frmSalary"  enctype="multipart/form-data"><input name="reset" value="Reset" type="reset" style='height:40px;'>
  ...
  <tbody>
    ...
    <tr>
		...
		<td class="head"><div $stylecomplete>Complete</div></td>
      	<td class="even"><input type="checkbox" $checked2 name="iscomplete" onclick="completeSalary(this.checked)" $stylecomplete></td>
    </tr>
...
//	$employeectrl = $this->getEmployeeListArray($expensesline_discount,$i);
	$expensesctrl = $this->getSelectSalarylist($expenseslist_id,$i);
...
EOF;
	...
	if($i == 0){
	echo "<tr><td colspan='8'>Please Add Salary Line.</td></tr>";
	}
...
EOF;
   ...
   * @access public
   */
  public function deleteSalary( $expenseslistmaster_id ) {
	...
	if (!rs){
		...
		$this->log->showLog(3,"Salary ($expenses_id) removed from database successfully!");
		return true;
		
	}
  } // end of member function deleteSalaryMaster
...
public function showSalaryTable(){
	...
	$this->log->showLog(3,"Showing Salary Table");
	$sql=$this->getSQLStr_AllSalary($wherestring," GROUP BY a.expenses_id ORDER BY expenses_date desc ",0);
	...
	<form action="salaryrpt.php" method="POST" name="frmSearch" onsubmit="return validateRpt();" target="_BLANK">
	...
	<select name="filter" onchange="showFld(this.value);">
	<option value=""></option>
	...
	<input type="submit" value="View"  style='height:40px;'>
	<input type="button" value="View Chart"  style='height:40px;' onclick="viewChart()">
	</td>
	</tr>
...
/**
   ...
   * @access public
   */
  public function getLatestSalaryID(){
...
public function getSelectSalary($id,$isitem='NY',$calledfunction="") {
...
/*
  ...
  public function getSelectSalarylist($id,$i) {
  ...
  public function getPriceSalarylist($id){
	$price = "0.00";
