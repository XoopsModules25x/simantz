<?php
/************************************************************************
Class Joindate.php - Copyright kfhoo
**************************************************************************/
...
class Joindate
{
  ...
  public function Joindate($xoopsDB,$tableprefix,$log){
  ...
  public function insertJoindate( ) {
   ...
   * @access public
   */
  public function getSqlStr_AllJoindate( $wherestring,  $orderbystring,  $startlimitno ) {
   ...
   $this->log->showLog(4,"Running Joindate->getSQLStr_AllJoindate: $sql");
  return $sql;
   ...
   * @access public
   */
  public function fetchJoindateInfo( $expenses_id ) {
    
	$this->log->showLog(3,"Fetching expenseslist detail into class Joindate.php.<br>");
		
	$sql="SELECT * FROM $this->tableexpenses
	where expenses_id=$expenses_id";
	
	$this->log->showLog(4,"Joindate->fetchJoindateInfo, before execute:" . $sql . "<br>");
	...
	if($row=$this->xoopsDB->fetchArray($query)){
		...
		$this->isactive=$row['isactive'];
	   	...
	   	$this->log->showLog(4,"Joindate->fetchJoindateInfo,database fetch into class successfully");
		return true;
	}
	else{
		return false;
	$this->log->showLog(4,"Joindate->fetchJoindateInfo,failed to fetch data into databases.");	
	}
  } // end of member function fetchJoindateInfo
  ...
  public function getInputForm( $type,  $expenses_id,$token ) {
	...
	else
	{
		...
		"<input name='title' value='Joindate' type='hidden'>".
		"<input name='btnView' value='View Record Info' type='submit'>".
		"</form>";
		


		$header="Edit Joindate";
...
<table style="width:300px;"><td><form onsubmit="return validateJoindate()" method="post"
 	action="expenses.php" name="frmJoindate"  enctype="multipart/form-data"><input name="reset" value="Reset" type="reset" style='height:40px;'>
  ...
  <tbody>
    ...
    <tr>
		...
		<td class="head"><div $stylecomplete>Complete</div></td>
      	<td class="even"><input type="checkbox" $checked2 name="iscomplete" onclick="completeJoindate(this.checked)" $stylecomplete></td>
    </tr>
...
<tr $displayadd>
	<td class="head">List Joindate :</td>
	<td class="odd">
...
//	$employeectrl = $this->getEmployeeListArray($expensesline_discount,$i);
	$expensesctrl = $this->getSelectJoindatelist($expenseslist_id,$i);
...
EOF;
	...
	if($i == 0){
	echo "<tr><td colspan='8'>Please Add Joindate Line.</td></tr>";
	}
...
EOF;
   ...
   * @access public
   */
  public function deleteJoindate( $expenseslistmaster_id ) {
	...
	if (!rs){
		...
		$this->log->showLog(3,"Joindate ($expenses_id) removed from database successfully!");
		return true;
		
	}
  } // end of member function deleteJoindateMaster
...
/**
   ...
   * @access public
   */
public function showJoindateTable(){
	if($this->start_date=="")
	...
	$this->log->showLog(3,"Showing Joindate Table");
	$sql=$this->getSQLStr_AllJoindate($wherestring," GROUP BY a.expenses_id ORDER BY expenses_date desc ",0);
	...
	<form action="joindaterpt.php" method="POST" name="frmSearch" onsubmit="return validateRpt();" target="_BLANK">
...
/**
   ...
   * @access public
   */
  public function getLatestJoindateID(){
...
public function getSelectJoindate($id,$isitem='NY',$calledfunction="") {
  ...
  public function getJoindatePrice($id){
...
/*
  ...
  public function getSelectJoindatelist($id,$i) {
  ...
  public function getPriceJoindatelist($id){
	$price = "0.00";
