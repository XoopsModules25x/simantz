...
function Header()
{
	...
	$this->Cell(0,4,"Stock Summary Report",0,1,'L');
	...
	*/
	...
	$header =array("No","Date","Employee","Type","Item","Qty","Accumulate Qty");
	$w=array(10,18,40,25,53,25,25); // total width = 285
...
function BasicTable($header,$data,$printheader)
{
	...
	//$w=array(10,18,40,25,53,20,30); // total width = 285
	$w=array(10,18,40,25,53,10,15,25); // total width = 285
...
if ($_POST){
	...
	if($transaction_type != "")
		$wherestring=$wherestring." AND a.type = '$transaction_type' ";
	...
	*/
	...
	$sql = "select * from (
		...
		order by a.product_id,a.movement_date ";
	...
	if($row['type'] == "Internal Use" || $row['type'] == "Sales" || $row['type'] == "Adjustment")
	$m_qty = $qty*(-1);
	else
	$m_qty = $qty;
	...
	$accumulate += $m_qty;
 	...
 	$data[]=array($i,$movement_date,$employee_name,$typename,$product_name,$qty,$uom,$accumulate);
