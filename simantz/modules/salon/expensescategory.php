...
$log = new Log();
$o = new Expensescategory($xoopsDB,$tableprefix,$log);
  ...
  case "create" :
	...
	if($o->insertCategory()){
		 $latest_id=$o->getLatestCategoryID();
			 redirect_header("expensescategory.php?action=edit&category_id=$latest_id",$pausetime,"Your data is saved, the new id=$latest_id");
		}
	else {
...
break;
  ...
  case "edit" :
	if($o->fetchCategory($o->category_id)){
		...
		redirect_header("expensescategory.php",3,"Some error on viewing your category data, probably database corrupted");
  ...
  case "update" :
	if ($s->check(false,$token,"CREATE_CAT")){
		...
		if($o->updateCategory()) //if data save successfully
			redirect_header("expensescategory.php?action=edit&category_id=$o->category_id",$pausetime,"Your data is saved.");
		else
			redirect_header("expensescategory.php?action=edit&category_id=$o->category_id",$pausetime,"Warning! Can't save the data, please make sure all value is insert properly.");
		}
	else{
		redirect_header("expensescategory.php?action=edit&category_id=$o->category_id",$pausetime,"Warning! Can't save the data, please make sure all value is insert properly.");
	}
  ...
  case "delete" :
	if ($s->check(false,$token,"CREATE_CAT")){
		if($o->deleteCategory($o->category_id))
			redirect_header("expensescategory.php",$pausetime,"Data removed successfully.");
		else
			redirect_header("expensescategory.php?action=edit&category_id=$o->category_id",$pausetime,"Warning! Can't delete data from database.");
	}
	else
		redirect_header("expensescategory.php?action=edit&category_id=$o->category_id",$pausetime,"Warning! Can't delete data from database.");
