...
{
  public $customer_id="Unknown";
  public $customer_name="Unknown";
  public $tel1="Unknown";
...
//    $this->Image('./images/visionlogobk.jpg', 263 , 11 , 20 , '' , 'JPG' , '');
    ...
    $this->Cell(0,17,"Customer Summary Report",1,0,'L');
    $this->Ln(4);
...
$pdf->AliasNbPages();
	...
	$pdf->header=array('No','Customer Name','Tel','H/P','Email','Address','Y/N');
	$sql="SELECT c.customer_name, c.tel1, c.tel2, c.email, c.address1, c.address2, c.postcode, c.city, a.area_name, c.country, c.isactive FROM $tablecustomer c inner join $tablearea a on c.state_id=a.area_id order by isactive desc, customer_name";
	...
	$i=$i+1;
$data[]=array($i,$row['customer_name'],$row['tel1'],$row['tel2'],$row['email'],$row['address1'].",".$row['address2'].",".$row['postcode'].",".$row['city'].",".$row['area_name'].",".$row['country'],$row['isactive']);
}
	//print header
